      ...
      if (info.kind === 'idl') {
        ...
        opts.importHook(importPath, info.kind, function (err, str) {
          ...
          case 'schema':
            ...
            mergeImportedAttrs(null, info.kind === 'schema' ? {types: [obj]} : obj);
            break;
          default:
...
function Parser(str, opts) {
  opts = opts || {};
  this._oneWayVoid = !!opts.oneWayVoid;
  this._reassignJavadoc = !!opts.reassignJavadoc;
  this._imports = [];
  this._tk = new Tokenizer(str);
  this._tk.next(); // Prime tokenizer.
}

Parser.prototype.getImports = function () { return this._imports; };

Parser.prototype.readProtocol = function () {
  ...
  while (tk.get().val !== '}') {
    if (tk.get().val === 'import') {
      ...
      if (typeAttrs.name) {
        ...
        if (attrs.messages[message.name]) {
        ...
        attrs.messages[message.name] = message.attrs;
      }
    }
  }
  tk.next({id: '(eof)'});
