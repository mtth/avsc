...
// TODO: backtick escaping for names.
// TODO: Namespacing when importing protocols.
...
function assemble(fpath, opts, cb) {
  ...
  function assemblePreamble(err, importedAttrs) {
    ...
    try {
      ...
      cb(new Error(f('%s when parsing %s', err.stack, fpath)));
    }
    cb(null, attrs);
  }
  ...
  function assembleImport() {
    try {
      var importKind = tk.next({id: 'name'}).val;
      var fpath = path.join(dpath, JSON.parse(tk.next({id: 'string'}).val));
      tk.next({val: ';'});
    } catch (err) {
      cb(err);
      return;
    ...
    if (importKind === 'idl') {
      ...
      fs.readFile(fpath, function (err, str) {
        ...
        case 'schema':
          ...
          break;
        default:
          assemblePreamble(tk.error('invalid import'));
        }
      });
    }
  }
  ...
  function readAnnotation(attrs) {
    ...
    attrs[parts.join('')] = JSON.parse(tk.next({id: 'json'}).val);
    tk.next({val: ')'});
    tk.next();
  }

  function readProtocol(attrs) {
    tk.get({val: 'protocol'});
    ...
    while (tk.get().val !== '}') {
      ...
      if (typeAttrs.name) {
        // This was a named type declaration.
  ...
  // Doesn't include the terminator.
  function readField() {
    ...
    // We must manually place it correctly.
    if (attrs.type.order) {
...
Tokenizer.prototype.get = function (opts) {
  if (opts && opts.id && opts.id !== this._token.id) {
    throw this.error(f('expected %s but got %s', opts.id, this._token.id));
...
Tokenizer.prototype.error = function (msg) {
  ...
  return new Error(f('%s at %d:%d', msg, lineNum, pos - lineStart + 1));
};

Tokenizer.prototype.assertDone = function () {
  this._skip();
