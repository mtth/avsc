...
Type.prototype.toString = function (val, noCheck) {
  if (val === undefined) {
    // Consistent behavior with standard `toString` expectations.
    return this.getSchema(true);
  }
  if (!noCheck && !this.isValid(val)) {
...
Type.prototype.inspect = function () {
  var obj = JSON.parse(this.getSchema(true)); // Slow, but only for debugging.
 ...
 *
 * Note the coercion in `clone` to support JSON-serialized `Buffer`s.
 *
 */
...
UnionType.prototype.clone = function (val, opts) {
  ...
  if (wrap === 2) {
    ...
    while (i < l && obj === undefined) {
      try {
        obj = this._types[i].clone(val, opts);
      } catch (err) {
        i++;
      }
    }
    ...
    if (keys.length === 1) {
      ...
      if (i !== undefined) {
        obj = this._types[i].clone(val[name], opts);
      }
    }
  }
  if (obj !== undefined) {
 ...
 *
 */
function resolveNames(schema, namespace, key) {
  namespace = schema.namespace || namespace;

  var name = schema[key || 'name'];
  if (!name) {
    throw new Error(f('missing name in %j', schema));
  }
  return {
    name: qualify(name),
