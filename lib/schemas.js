...
function assemble(fpath, opts, cb) {
  ...
  var imports = [];
  var attrs = {types: [], messages: {}};
  opts.imports[fpath] = true;
  ...
  function assembleImports(err, importAttrs) {
    ...
    if (importAttrs) {
      // Merge in any imported attributes.
      attrs.types = attrs.types.concat(importAttrs.types || []);
    ...
    if (!info) {
      ...
      fs.readFile(info.path, function (err, str) {
        if (err) {
          cb(err);
          return;
        }
        ...
        switch (info.kind) {
        case 'protocol':
          assembleImports(null, attrs);
          break;
        case 'schema':
          assembleImports(null, {types: [attrs]});
          break;
        default:
          assembleImports(f('invalid import kind: %s', info.kind));
        }
      });
    }
  }
...
Tokenizer.prototype.error = function (msg) {
  ...
  pos -= lineStart;
  return new Error(f('%s at %d:%d in %s', msg, lineNum, pos, this._fpath));
};
