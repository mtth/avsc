...
function assemble(fpath, opts, cb) {
  ...
  function readMessage(protocolAttrs, responseAttrs, oneWay) {
    var messageAttrs = {};
    // Inspired by order handling below, we take the liberty to make message
    // javadocs more useful by moving any doc from the response to the message.
    messageAttrs.response = transferKey('doc', responseAttrs, messageAttrs);
    if (oneWay) {
  ...
  function readField() {
    ...
    // We must manually place it correctly and fix up the type. We do the same
    // for javadocs since they would be inconvenient to use for fields.
    attrs.type = transferKey('doc', attrs.type, attrs);
    attrs.type = transferKey('order', attrs.type, attrs);
    while (tk.get().val === '@') {
      readAnnotation(attrs);
    }
    tk.addDoc(attrs);
...
Tokenizer.prototype.addDoc = function (attrs) {
    ...
    case '*':
      ...
      while ((c = str.charAt(this._pos++))) {
        if (c === '*' && str.charAt(this._pos) === '/') {
          this._pos++;
          if (pos !== undefined) {
            this._doc = fromJavadoc(str.slice(pos, this._pos - 2));
          }
          return this._skip();
        }
      }
 ...
 * Clean up Javadoc.
 *
 * This is very simple and simply removes the line prefixes and leading /
 * trailing empty lines. It's better to be conservative with formatting rather
 * than risk losing information.
 *
 */
function fromJavadoc(str) {
  str = str.replace(/^[ \t]+|[ \t]+$/g, '');
  var lines = str.split('\n').map(function (line, i) {
 ...
 *
 * If the source becomes an object with only a type attribute set, it will be
 * transformed to a string.
 *
 */
function transferKey(key, from, to) {
