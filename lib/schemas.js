...
function assemble(fpath, opts, cb) {
  ...
  opts = opts || {};
  opts.importer = opts.importer || createImporter();
  ...
  var tk; // Tokenizer.
  opts.importer(fpath, function (err, str) {
    if (err) {
      cb(err);
      return;
    }
    ...
    var pat = /^\s*import\s+(idl|protocol|schema)\s+"([^"]+)"\s*;$/gm;
    tk = new Tokenizer(str.replace(pat, function (all, kind, fname) {
      imports.push({kind: kind, path: path.join(dpath, fname)});
  ...
  function assembleImports(err, importAttrs) {
    ...
    if (!info) {
      ...
      opts.importer(info.path, function (err, str) {
        if (err) {
          cb(err);
          return;
        }
