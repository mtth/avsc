...
function assemble(fpath, opts, cb) {
  ...
  opts.importHook(fpath, 'idl', function (err, str) {
    ...
    try {
      var protocol = parseProtocol(str, opts);
    } catch (err) {
  ...
  function fetchImports() {
    ...
    if (!info) {
      ...
      cb(null, JSON.parse(JSON.stringify(schema)));
    } else {
        ...
        opts.importHook(importPath, info.kind, function (err, str) {
          ...
          switch (info.kind) {
            ...
            case 'schema':
              ...
              mergeImportedAttrs(null, schema);
              break;
            default:
  ...
  function mergeImportedAttrs(err, importedAttrs) {
    ...
    try {
      Object.keys(importedAttrs.messages || {}).forEach(function (name) {
        ...
        schema.messages[name] = importedAttrs.messages[name];
      });
    } catch (err) {
      cb(err);
      return;
 ...
 *
 */
function parseProtocol(str, opts) {
  var parser = new Parser(str, opts);
  return {schema: parser._readProtocol(), imports: parser._imports};
}
...
Tokenizer.prototype.addJavadoc = function (schema) {
  if (this._doc === undefined || schema.doc !== undefined) {
    return;
  }
...
Parser.prototype._readProtocol = function () {
  ...
  while (tk.get().val === '@') {
    this._readAnnotation(schema);
  }
  tk.addJavadoc(schema);
  tk.get({val: 'protocol'});
  schema.protocol = tk.next({id: 'name'}).val;
  tk.next({val: '{'});
  tk.next();
  while (tk.get().val !== '}') {
...
Parser.prototype._readAnnotation = function (schema) {
  var tk = this._tk;
  tk.get({val: '@'});
...
Parser.prototype._readMessage = function (responseAttrs) {
  ...
  tk.get({val: ';'});
  tk.next();
  return {name: name, schema: messageAttrs};
};
...
Parser.prototype._readField = function () {
  ...
  if (this._reassignJavadoc) {
    schema.type = reassignJavadoc(schema.type, schema);
  }
  while (tk.get().val === '@') {
    this._readAnnotation(schema);
  }
  tk.addJavadoc(schema);
  schema.name = tk.get({id: 'name'}).val;
  if (tk.next().val === '=') {
...
Parser.prototype._readFixed = function (schema) {
  var tk = this._tk;
  schema.type = tk.get({val: 'fixed'}).val;
...
Parser.prototype._readMap = function (schema) {
  var tk = this._tk;
  schema.type = tk.get({val: 'map'}).val;
  tk.next({val: '<'});
  tk.next();
...
Parser.prototype._readArray = function (schema) {
  var tk = this._tk;
  schema.type = tk.get({val: 'array'}).val;
  tk.next({val: '<'});
  tk.next();
...
Parser.prototype._readEnum = function (schema) {
  var tk = this._tk;
  schema.type = tk.get({val: 'enum'}).val;
  schema.name = tk.next({id: 'name'}).val;
  tk.next({val: '{'});
  schema.symbols = [];
  do {
...
Parser.prototype._readUnion = function (schema) {
  var tk = this._tk;
  var arr = [];
  tk.get({val: 'union'});
  tk.next({val: '{'});
...
Parser.prototype._readRecord = function (schema) {
  var tk = this._tk;
  schema.type = tk.get({id: 'name'}).val;
  schema.name = tk.next({id: 'name'}).val;
  schema.fields = [];
  tk.next({val: '{'});
  while (tk.next().val !== '}') {
