...
// TODO: Remove custom record constructor (since this can now be accomplished
// with logical types).
// TODO: Add `toJSON` function for abstract long.
// TODO: Rename coerceBuffers _copy option (since it's now used as a flag to
// figure out which conversion is happening between JSON string and in-memory
// representation).
...
function createType(attrs, opts) {
  ...
  if (attrs.logicalType && !LOGICAL_TYPE) {
    ...
    if (DerivedType) {
      var registry = {};
      Object.keys(opts.registry).map(function (key) {
        registry[key] = opts.registry[key];
      });
      try {
        return new DerivedType(attrs, opts);
      } catch (err) {
        // Fall through to underlying type as the spec mandates.
        LOGICAL_TYPE = null;
...
Type.prototype.toString = function (val) {
  ...
  return JSON.stringify(this._copy(val, {coerceBuffers: 3}));
};
...
UnionType.prototype._copy = function (val, opts) {
  var wrap = opts && opts.wrapUnions | 0;
  if (wrap === 1) {
...
function LogicalType(attrs, opts, Types) {
  ...
  if (Types && !~Types.indexOf(this._underlyingType.constructor)) {
    throw new Error(f('invalid type for %s: %s', this, this._underlyingType));
  }
}
...
LogicalType.prototype._write = function (tap, obj) {
  this._underlyingType._write(tap, this._toValue(obj));
};

LogicalType.prototype._check = function (obj, cb) {
  return this._underlyingType._check(this._toValue(obj), cb);
};

LogicalType.prototype._copy = function (obj, opts) {
  var type = this._underlyingType;
  switch ((opts && opts.coerceBuffers) | 0) {
    case 3: // to JSON.
      return type._copy(this._toValue(obj), opts);
    case 2: // from JSON.
      return this._fromValue(type._copy(obj, opts));
    default: // copy.
      return this._fromValue(type._copy(this._toValue(obj), opts));
  }
};
...
LogicalType.prototype.toJSON = function () {
  return {type: this._underlyingType};
};

// Methods to be implemented.
LogicalType.prototype._fromValue = util.abstractFunction;
LogicalType.prototype._toValue = util.abstractFunction;
LogicalType.prototype._resolve = util.abstractFunction;
...
AbstractLongType.prototype._copy = function (val) {
...
function Field(attrs, opts) {
  ...
  if (value !== undefined) {
    ...
    var type = this._type;
    var val = type._copy(value, {coerceBuffers: 2, wrapUnions: true});
