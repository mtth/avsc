...
BlockDecoder.prototype._decodeHeader = function () {
  ...
  try {
    ...
    this._type = types.Type.fromSchema(schema, this._typeOpts);
  } catch (err) {
    this.emit('error', err);
    return;
  }
...
function FrameEncoder(type, opts) {
  ...
  this._tap = new Tap(new Buffer(opts.bufferSize || this._frameSize + 1024));
}
...
BlockEncoder.prototype.flush = function () { this._flushChunk(); };
