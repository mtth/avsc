...
RawDecoder.prototype._transformChunk = function (chunk, encoding, cb) {
  ...
  var len = buf.length;
  var pos;
  while ((pos = tap.pos) < len) {
    var obj = this._readObj.call(tap);
    if (this._includeEncoding) {
      this.push(new Pair(obj, buf.slice(pos, tap.pos)));
    } else {
      this.push(obj);
    }
  }
...
Encoder.prototype._writeChunk = function (obj, encoding, cb) {
  var tap = this._tap;
  var pos = tap.pos;
...
Encoder.prototype._flushChunk = function (pos) {
  var tap = this._tap;
  pos = pos || tap.pos;
...
Encoder.prototype._addBlockCallback = function () {
  var self = this;
  var index = this._index++;
