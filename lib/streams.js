...
Decoder.isContainerFile = function (path) {
  var fd = fs.openSync(path, 'r');
  var buf = new Buffer(4);
  var pos = 0;
  while (pos < 4) {
    pos += fs.readSync(fd, buf, pos, 4 - pos);
  }
  fs.closeSync(fd);
  return MAGIC_BYTES.equals(buf);
};
...
Decoder.prototype._decodeHeader = function () {
  ...
  try {
    ...
    if (this.readerType) {
      this._readObject = this.readerType.createAdapter(this.writerType)._read;
    } else {
      this._readObject = this.writerType._read;
    }
  } catch (err) {
    this.emit('error', err);
    return;
  ...
  this._record = this.writerType.random();
  // this._readObject = function () { self.writerType._fill.call(this, self._record); }
  this._fillObject = this.writerType._fill;
  // console.log(this._fillObject.toString());
  // process.exit(0);
...
Decoder.prototype._read = function () {
  ...
  if (!blockData) {
    if (this._finished && !this._pending) {
      setTimeout(function () { self.push(null); }, 0);
    } else {
      this._needPush = true;
    }
    return;
  ...
  while (tap.pos < len) {
    // this.writerType._skip.call(tap);
    this.push(this._readObject.call(tap));
    // this._fillObject.call(tap, this._record);
...
function RawDecoder(opts) {
  ...
  this.on('pipe', function (input) {
    if (this._input) {
      this.emit('error', new AvscError('reusing decoder'));
    }
    this._input = input;
  });
}
...
RawDecoder.prototype._transform = function (chunk, encoding, cb) {
  if (!this.writerType) {
    if (this._input.writerType) {
      this._writerType = this._input.writerType;
    } else {
      this.emit('error', new AvscError('cannot infer writer type'));
    }
  }
...
function RawEncoder(opts) {
  ...
  this.writerType = opts.writerType || null; // Maybe inferred below.
...
RawEncoder.prototype._transform = function (obj, encoding, cb) {
  ...
  cb(null, this.writerType.encode(obj));
};
