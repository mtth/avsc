...
Decoder.prototype._decodeHeader = function () {
  ...
  try {
    ...
    this._writerType = types.Type.fromSchema(schema, this._parseOpts);
  } catch (err) {
    this.emit('error', err);
    return;
  }
...
Decoder.prototype._writeBlockChunk = function (chunk, encoding, cb) {
  ...
  while ((block = getBlock(tap))) {
    ...
    this._pending++;
    this._decompress(block.data, decompressCb(this, this._));
  }
  cb();

};

Decoder.prototype._writeFragmentChunk = function (chunk, encoding, cb) {
