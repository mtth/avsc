...
Protocol.prototype.createEmitter = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    return new StatefulEmitter(this, readable, writable);
  }
};
...
Emitter.prototype._getAdapter = function (hres) {
  ...
  if (serverBuf.equals(clientBuf)) {
    adapter = new Adapter(this._ptcl, this._ptcl, clientBuf);
  } else {
...
Listener.prototype._getAdapter = function (hreq) {
  ...
  if (clientBuf.equals(serverBuf)) {
    adapter = new Adapter(this._ptcl, this._ptcl, serverBuf);
  } else {
