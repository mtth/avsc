...
// TODO: Add `call` event on emitters and listeners with arguments: stats, req,
// err, res (where stats is an Avro record with fields like time taken, status,
// etc.)?
// TODO: Split FrameDecoder and FrameEncoder from MessageEmitter and
// MessageListener? They are better added by the protocol (inside the
// `createXXX` methods), with the default same as current behavior. This would
// make it simple to implement adapters to non-standard wire formats.
// TODO: Optimize FrameEncoder by avoiding the extra copy on transform.
...
function createProtocol(attrs, opts) {
  ...
  return new Protocol(
    name,
    types.createType(['string'], {wrapUnions: opts.wrapUnions}),
    messages,
    opts.registry || {},
    !!opts.strictErrors
  );
 ...
 *
 */
function Protocol(name, systemErrorType, messages, types, strictErrors, ptcl) {
  this._name = name;
  this._systemErrorType = systemErrorType;
  this._messages = messages;
  this._types = types;
  this._strictErrors = strictErrors;
  this._parent = ptcl;

  // Cache a string instead of the buffer to avoid retaining an entire slab.
  this._hashString = this.getFingerprint().toString('binary');
  ...
  this._onListenerCall = function (name, req, cb) {
    ...
    if (handler) {
      // We only pass in the response callback if the message isn't one way.
      try {
        handler.call(self, req, this, cb && onResponse);
      } catch (err) {
        ...
        if (cb) {
          onResponse(self._toSystemError(err));
        }
      }
    } else if (cb) {
      // This (listening) protocol hasn't implemented this message, we send an
      // error message back if possible (cb will undefined if the message is
      // one-way and the transport is stateful).
      onResponse(self._toSystemError(f('unhandled message: %s', name)));
    }

    function onResponse(err, res) {
      if (!self._strictErrors) {
        if (err === null) {
          // We allow passing null instead of undefined, as is the case
          // everywhere in node.js world. We don't allow this in strict mode
          // because it can lead to ambiguities in the rare case where the null
          // type is explicitly added to the error union.
          err = undefined;
        } else {
          // We also convert system errors from actual errors to strings. This
          // way clients can respond with the usual error as first argument
          // which is more natural, especially when using unwrapped unions.
          var errorType = self._messages[name].getErrorType();
          if (isError(err)) {
            err = err.message;
            if (Type.isType(errorType, 'union:wrapped')) {
              err = {string: err};
            }
          }
        }
...
Protocol.prototype._toSystemError = function (err) {
  if (typeof err != 'string') {
    err = err.message;
  }
  if (Type.isType(this._systemErrorType, 'union:wrapped')) {
    err = {string: err};
  }
  return err;
};
...
Protocol.prototype.emit = function (name, req, ee, cb) {
  if (!ee || !ee._ptcl || ee._ptcl._hashString !== this._hashString) {
    throw new Error('invalid emitter');
  }

  var message = this._messages[name];
  if (!message) {
    // We could also throw asynchronously here, but this is more consistent
    // with `on`'s behavior when listening to a missing message.
    throw new Error(f('unknown message: %s', name));
  }

  var self = this;
  ee._emit(message, req, function (err, res) {
    var noError = err === undefined;
    if (!self._strictErrors) {
      // We perform the reverse operations to those in `_onListenerCall`.
      if (noError) {
        err = null;
      } else {
        var errorType = self._messages[name].getErrorType();
        if (Type.isType(errorType, 'union:unwrapped')) {
          if (typeof err == 'string') {
            err = new Error(err);
          }
    ...
    if (cb) {
      ...
      ee.emit('error', wrapError('unhandled error', err));
    }
  });
  return ee._pending;
};
...
Protocol.prototype.createEmitter = function (transport, opts, cb) {
  ...
  if (typeof transport == 'function') {
    ee = new StatelessEmitter(this, transport, opts);
  } else {
    ...
    ee = new StatefulEmitter(this, readable, writable, opts);
  }
  if (cb) {
    ee.once('eot', cb);
  }
...
Protocol.prototype.toString = function () { return this.getSchema(true); };
...
function MessageEmitter(ptcl, opts) {
  ...
  this._id = 1;
  this._pending = 0; // To optimize lookup.
  this._calls = {};
...
MessageEmitter.prototype._addCall = function (message, req, cb) {
  var id = this._id++;
  if (cb) {
    this._pending++;
    this._calls[id] = {
...
MessageEmitter.prototype._popCall = function (id) {
  this._pending--;
...
MessageEmitter.prototype._clearCalls = function () {
  this._pending = 0;
  var calls = Object.keys(this._calls).map(function (id) {
...
MessageEmitter.prototype._createHandshakeRequest = function (
  ...
  return new HandshakeRequest(
    getHash(this._ptcl),
    noPtcl ? null : this._ptcl.getSchema(),
...
MessageEmitter.prototype._finalizeHandshake = function (tap, handshakeReq) {
  ...
  if (handshakeReq.clientProtocol && res.match === 'NONE') {
    ...
    var buf = res.meta && res.meta.error;
  ...
  if (res.serverHash && res.serverProtocol) {
    ...
    hashString = res.serverHash.toString('binary');
    if (!canResolve(this, hashString)) {
      this._generateResolvers(
        hashString,
        createProtocol(JSON.parse(res.serverProtocol))
      );
    }
...
StatelessEmitter.prototype._emit = function (message, req, cb) {
  if (this._destroyed) {
    utils.nextTick(cb, this._ptcl._toSystemError('emitter destroyed'));
    return;
  }
  ...
  var id = this._addCall(message, req, cb);
  var self = this;
  utils.nextTick(emit, false);
  ...
  function emit(retry) {
    ...
    try {
      ...
      done(self._ptcl._toSystemError(err));
      return;
    }
    ...
    var writable = self._writableFactory(function onReadable(readable) {
      ...
      readable
        .pipe(new FrameDecoder({noEmpty: !message._oneWay}))
        .on('error', function (err) { done(self._ptcl._toSystemError(err)); })
    ...
    var encoder = new FrameEncoder({frameSize: self._frameSize});
    encoder.pipe(writable);
    encoder.end(tap.getValue());
  }

  function done(err, res) {
    self._popCall(id).cb.call(self._ptcl, err, res);
    if (self._destroyed) {
      self.destroy();
    }
  }
...
StatelessEmitter.prototype.destroy = function (noWait) {
  ...
  if (noWait && pending) {
    ...
    this._clearCalls().forEach(function (call) {
      call.cb(this._ptcl._toSystemError('interrupted'));
    }, this);
  }
...
function StatefulEmitter(ptcl, readable, writable, opts) {
  ...
  this._encoder = new FrameEncoder({frameSize: this._frameSize});
  ...
  function onMessageData(buf) {
    ...
    try {
      ...
      self.emit('error', wrapError('invalid metadata', err));
      return;
    }

    var call = self._popCall(id);
    if (call === undefined) {
    ...
    try {
      ...
      call.cb(self._ptcl._toSystemError(err));
      return;
    }
    call.cb.apply(undefined, args);
    if (self._destroyed) {
      self.destroy();
    }
  }
...
StatefulEmitter.prototype._emit = function (message, req, cb) {
  ...
  var id = this._addCall(message, req, message.isOneWay() ? undefined : cb);
  var self = this;
  if (this._started) {
    process.nextTick(emit);
  } else {
    this.once('_start', emit);
  }
  ...
  function emit() {
    ...
    try {
      ...
      self._popCall(id);
      utils.nextTick(cb, self._ptcl._toSystemError(err));
      return;
    }
...
StatefulEmitter.prototype.destroy = function (noWait) {
  ...
  var pending = this._pending;
  if (pending && !noWait && !this._ended && this._started) {
...
MessageListener.prototype._validateHandshake = function (reqTap, resTap) {
      ...
      if (emitterPtclString) {
        try {
          this._generateResolvers(
            ...
            createProtocol(JSON.parse(emitterPtclString))
          );
        } catch (err) {
          validationErr = err;
        }
  ...
  var handshakeRes = new HandshakeResponse(
    ...
    serverMatch ? null : this._ptcl.getSchema(),
    serverMatch ? null : getHash(this._ptcl),
    validationErr ? {error: new Buffer(validationErr.message)} : null
  );
...
MessageListener.prototype._encodeSystemError = function (tap, err) {
  ...
  safeWrite(tap, this._ptcl._systemErrorType, err);
};
...
MessageListener.prototype._encodeArguments = function (
  ...
  try {
    ...
    tap.pos = pos;
    this._encodeSystemError(tap, this._ptcl._toSystemError(err));
  }
};
...
MessageListener.prototype.destroy = function (noWait) {
  if (!this._destroyed) {
    ...
    // the readable stream (via `FrameDecoder`'s `unpipe` handler).
    this._readable.unpipe(this._decoder);
  }
...
function StatelessListener(ptcl, readableFactory, opts) {
  ...
  function onRequestData(buf) {
    ...
    if (!self._validateHandshake(reqTap, self._tap)) {
      onResponse(self._ptcl._toSystemError('invalid handshake'));
      return;
    }
    ...
    try {
      ...
      onResponse(self._ptcl._toSystemError(err));
      return;
    }

    if (self._message._oneWay) {
      self.emit('_call', name, req);
      onResponse(undefined, null);
    } else {
  ...
  function onResponse(err, res) {
    ...
    if (!self._message) {
      // Since we don't have a message yet, this means that an error occurred
      // and that it's a system error.
...
function FrameEncoder(opts) {
  ...
  this._frameSize = (opts && opts.frameSize) || 1024;
...
FrameEncoder.prototype._transform = function (buf, encoding, cb) {
  var frames = [];
 ...
 *
 */
function FrameDecoder(opts) {
  stream.Transform.call(this);
  this._buf = new Buffer(0);
  this._bufs = [];
  this._length = 0;
  this._empty = !!(opts && opts.noEmpty);
...
FrameDecoder.prototype._flush = function () {
  ...
  } else if (this._length || this._buf.length) {
 ...
 *
 */
function canResolve(ee, hashString) {
  var resolvers = ee._resolvers[hashString];
  return !!resolvers || hashString === ee._ptcl._hashString;
}

/**
 * Retrieve resolvers for a given hash string.
 *
 * @param ee {MessageEmitter|MessageListener}
 * @param hashString {String}
 * @param message {Message}
 *
 */
function getResolvers(ee, hashString, message) {
  if (hashString === ee._ptcl._hashString) {
    return message;
  }
  var resolvers = ee._resolvers[hashString];
