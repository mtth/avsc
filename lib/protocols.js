...
var BOOLEAN_TYPE = Type.create('boolean', OPTS);

var MAP_BYTES_TYPE = Type.create({type: 'map', values: 'bytes'}, OPTS);

var STRING_TYPE = Type.create('string', OPTS);

var HANDSHAKE_REQUEST_TYPE = Type.create({
...
var HANDSHAKE_RESPONSE_TYPE = Type.create({
...
function Protocol(name, messages, types, handlers) {
  if (types === undefined) {
    ...
    return Protocol.create(name, messages);
  }
...
Protocol.prototype.createListener = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    var listener = new StatefulListener(this, readable, writable, opts);
    if (!objectMode) {
 ...
 *
 */
Protocol.create = function (schema, opts) {
  opts = opts || {};
  ...
  if (schema.types) {
    schema.types.forEach(function (obj) { Type.create(obj, opts); });
  }

  var messages = {};
  if (schema.messages) {
    Object.keys(schema.messages).forEach(function (key) {
      messages[key] = new Message(key, schema.messages[key], opts);
    });
  }
  return new Protocol(name, messages, opts.registry || {});
};

Protocol.assembleAttributes = utils.abstractFunction;

Protocol.discoverAttributes = function (transport, opts, cb) {
  if (cb === undefined && typeof opts == 'function') {
    cb = opts;
    opts = undefined;
  }

  var emitter = Protocol.create({protocol: 'Empty'}).createEmitter(transport, {
  ...
  emitter
    .once('handshake', function (hreq, hres) {
      emitter.destroy(true); // Prevent further requests on this emitter.
...
Protocol.parseAttributes = utils.abstractFunction;

// Basic way of exposing these constructors.
...
MessageEmitter.prototype.emitMessage = function (name, reqEnv, opts, cb) {
  ...
  if (this._destroyed) {
    ...
    if (!msg) {
      err = new Error(f('unknown message: %s', name));
    } else {
      try {
...
MessageEmitter.prototype._getAdapter = function (hres) {
  ...
  var serverPtcl = Protocol.create(
...
MessageListener.prototype._getAdapter = function (hreq) {
  ...
  var clientPtcl = Protocol.create(
 ...
 *
 */
function Message(name, schema, opts) {
  opts = opts || {};
  ...
  this._responseType = Type.create(schema.response, opts);

  var errors = schema.errors || [];
  errors.unshift('string');
  this._errorType = Type.create(errors, opts);
...
Message.prototype.inspect = Message.prototype.toJSON = function () {
  ...
  if (errorTypes.length > 1) {
    obj.errors = Type.create(errorTypes.slice(1));
  }
  if (this._oneWay) {
    obj['one-way'] = true;
  }
...
function Registry(ctx, prefixLength) {
  ...
  this._mask = ~0 >>> (prefixLength | 0); // 32 bits by default.
