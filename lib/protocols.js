...
// TODO: Add a `schema` field on protocols to support sending arbitrary
// attributes over the wire. Unlike types, this doesn't cause issues with
// storing arbitrarily many schemas (protocols can't be nested).
...
function Client(ptcl, opts) {
  ...
  if (opts.remoteSchemas) {
    Adapter._populateCache(this._cache$, ptcl, opts.remoteSchemas, true);
  }
...
Client.prototype.getRemoteSchemas = function () {
  return Adapter._getRemoteSchemas(this._ptcl$, this._cache$);
};
...
function Server(ptcl, opts) {
  ...
  if (opts.remoteSchemas) {
    Adapter._populateCache(this._cache, ptcl, opts.remoteSchemas, false);
  }
...
function MessageEmitter(client, opts) {
  ...
  var clientPtcl = client._ptcl$;
  var fgpt = opts.serverFingerprint; // Deprecated.
  var schema, serverPtcl;
  if (opts.remoteSchema) {
    schema = opts.remoteSchema;
    serverPtcl = Protocol.forSchema(schema, {wrapUnions: true});
    fgpt = serverPtcl.getFingerprint();
    if (!cache[fgpt]) {
      cache[fgpt] = new Adapter(clientPtcl, serverPtcl, fgpt, schema);
    }
  } else if (!fgpt) {
    fgpt = clientPtcl.getFingerprint();
  }
  var adapter = cache[fgpt];
  if (!adapter) {
    // This might happen even if the server fingerprint option was set, in
    // cases where the cache doesn't contain the corresponding adapter. In this
    // case we fall back to the client's protocl (as mandated by the spec).
    // Note however, that we don't populate a schema in the adapter, this is to
    // signal that this schema shouldn't be included as a remote schema yet.
    fgpt = clientPtcl.getFingerprint();
    schema = clientPtcl.getSchema({exportAttrs: true});
    adapter = cache[fgpt] = new Adapter(clientPtcl, clientPtcl, fgpt);
  }
...
MessageEmitter.prototype._createHandshakeRequest = function (adapter, noPtcl) {
  var ptcl = this._client.getProtocol();
  return {
...
MessageEmitter.prototype._getAdapter = function (hres) {
  ...
  var serverPtcl = Protocol.forSchema(schema, {wrapUnions: true});
  adapter = new Adapter(this._client._ptcl$, serverPtcl, fgpt, schema);
...
MessageListener.prototype._getAdapter = function (hreq) {
  ...
  var clientPtcl = Protocol.forSchema(schema, {wrapUnions: true});
  adapter = new Adapter(clientPtcl, this._server.getProtocol(), fgpt, schema);
...
MessageListener.prototype.getProtocol = util.deprecate(
  ...
  'use `getServer().getProtocol()` instead'
);
 ...
 */
function Adapter(clientPtcl, serverPtcl, fingerprint, schema) {
...
Adapter._populateCache = function (cache, ptcl, schemas, isClient) {
  Object.keys(schemas).forEach(function (fgpt) {
    ...
    if (isClient) {
      ...
      clientPtcl = Protocol.forSchema(schema, {wrapUnions: true});
      serverPtcl = ptcl;
    }
    cache[fgpt] = new Adapter(clientPtcl, serverPtcl, fgpt, schema);
  });
...
Adapter._getRemoteSchemas = function (ptcl, cache) {
  var schemas = {};
  Object.keys(cache).forEach(function (hs) {
    // Note that we might also include the local protocol if it is also remote
    // (i.e. if its schema field is defined).
    var schema = cache[hs]._schema;
    if (schema) {
      schemas[hs] = schema;
    }
  });
...
Adapter.prototype._decodeRequest = function (buf) {
  if (!this._schema) {
    // This will happen if this is the client's default adapter, the server's
    // protocol matches,  and this is the first response received. In this case
    // we add the schema to indicate that this protocol is also remote.
    this._schema = this._clientPtcl.getSchema({exportAttrs: true});
  }
  var tap = new Tap(buf);
...
Adapter.prototype._decodeResponse = function (buf, wres) {
  if (!this._schema) {
    ...
    // protocol matches,  and this is the first response received. In this case
    // we add the schema to indicate that this protocol is also remote.
    this._schema = this._serverPtcl.getSchema({exportAttrs: true});
  }
  var tap = new Tap(buf);
