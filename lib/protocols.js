...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  return emitter._emit(wreq, opts, function (err, wres) {
    ...
    if (err) {
      if (self._strict$) {
        err = errType.clone(getErrorDescription(err), {wrapUnions: true});
      }
      done(err);
      return;
    }
...
MessageEmitter.prototype._emit = function (wreq, opts, cb) {
  ...
  process.nextTick(function () {
    if (wreq.getMessage().isPing()) {
      ...
      chainFns(self._client._fns$, self, [wreq], target, source);
    }
  });
    ...
    if (self._hook) {
      ...
      if (!handler) {
        ...
        try {
          ...
          // unnoticed.
          cb(toRpcError('INTERNAL_SERVER_ERROR', err));
        }
      }
    }
  }
...
/**
 * Retrieve information about an error in a compatible-ish way.
 *
 * @param err {Error} Any error.
 */
