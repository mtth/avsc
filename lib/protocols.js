...
Client.prototype.use = function (proxy) {
  this._fns$.push(proxy);
  return this;
};
...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  chainFns(
    ...
    emitter,
    [wreq],
    function (wreq, cb) { this._emit(wreq, opts, cb); },
    function (err, wres) {
...
/** Middleware logic. */
function chainFns(fns, ctx, args, handler, done) {
  var cbs = [];
  forward(0);
  ...
  function forward(pos) {
    ...
    if (pos < fns.length) {
      ...
      handler.apply(ctx, args.concat(function () {
        if (isDone) {
          err = new Error('duplicate middleware handler call');
        }
        isDone = true;
  ...
  function backward() {
    ...
    if (cb) {
      ...
      done.apply(ctx, [null].concat(args));
    }
  }
}
