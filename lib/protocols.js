...
function Message(name, reqType, errType, resType, isOneWay, doc) {
  ...
  this._requestType = reqType; // TODO: Shorten property names.
...
/** Factory function, doing schema validation. */
...
// Wrapper classes used in client and server middleware.
...
WrappedRequest.fromArgs = function (msg, args) {
  ...
  return new WrappedRequest(msg, {}, new (Record.bind.apply(Record, args))());
};
...
WrappedRequest.prototype._getValidationError = function () {
  var reqType = this._msg.getRequestType();
  var req = this._req;
  if (reqType.isValid(req)) {
    return undefined;
  }
  var err = new ValidationError(reqType);
  ...
  reqType.isValid(req, {errorHook: errorHook});
  return err;
};
...
// Deprecated protocol methods.

Protocol.prototype.subprotocol = util.deprecate(
  function () {
    var server = this.createServer();
    server.install(this._server);
    return new Protocol(
      this._name,
      this._messages,
      this._types,
  ...
  'user `protocol.createServer` and `server.install` instead'
);

Protocol.prototype.createEmitter = util.deprecate(
  function (transport, opts) {
    return this.createClient(transport, opts).getEmitter();
  },
...
Protocol.prototype.createListener = util.deprecate(
  function (transport, opts) {
    var server = this.createServer(opts);
    server.install(this._server);
    return server.createListener(transport, opts);
  },
...
Protocol.prototype.emit = util.deprecate(
  function (name, req, emitter, cb) {
    ...
    return client.getEmitter().getPending();
  },
...
Protocol.prototype.on = util.deprecate(
  function (name, handler) {
    ...
    this._server.onMessage(name, function (req, cb) {
      return handler.call(self, req, this, cb); // TODO: Strict error handling.
    });
    return this;
  },
...
Protocol.prototype.getHandler = util.deprecate(
  function (name) {
    var handler = this._server.getHandlers()[name];
    if (!handler) {
      return undefined;
    }
...
function discoverProtocolSchema(transport, opts, cb) {
  ...
  var client = ptcl.createClient(transport, {
...
/** Thin-ish wrapper around an emitter. */
function Client(ptcl, transport, opts) {
  // We have to suffix these properties to be safe, since the message names
  // aren't prefixed with clients (unlike servers).
  this._ptcl$ = ptcl;
  this._emitter$ = this._createEmitter(transport, opts);
  this._proxies$ = []; // Middleware.

  this._strict = opts && !!opts.strictErrors;
...
Client.prototype.getEmitter = function () {
...
Client.prototype.emitMessage = function (name, req, opts, cb) {
  ...
  return this._emitter$._emitMessage(wreq, opts, function (err, wres) {
    var errType = msg.getErrorType();
...
Client.prototype._createEmitter = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    var emitter = new StatefulEmitter(this, readable, writable, opts);
    if (!objectMode) {
...
Client.prototype._createMessageHandler = function (msg) {
  ...
  return function (/* fields ... , [opts,] [cb] */) {
    ...
    this._emitter$._emitMessage(wreq, opts, cb);
  };
};
...
Server.prototype.getHandlers = function () { return this._handlers; };

Server.prototype.getActiveListeners = function () {
...
Server.prototype.install = function (server) {
  this._handlers = Object.create(server.getHandlers(), this._handlers);
};

Server.prototype.onMessage = function (name, handler) {
  getExistingMessage(this.getProtocol(), name); // Check existence.
...
Server.prototype._createMessageHandler = function (message) {
  ...
  return function (handler) {
    this._handlers[message.getName()] = function (req, ee, cb) {
...
MessageEmitter.prototype.emitMessage = util.deprecate(
  function (name, reqEnv, opts, cb) {
    ...
    return this._emitMessage(wreq, opts, function (err, wres) {
      var env;
      if (!err) {
        env = {
...
MessageEmitter.prototype._emitMessage = function (wreq, opts, cb) {
  if (cb === undefined && typeof opts == 'function') {
    cb = opts;
    opts = undefined;
  }
...
function StatelessEmitter(client, writableFactory, opts) {
  ...
  if (!opts || !opts.noPing) {
    ...
    var wreq = new WrappedRequest(Message.PING, {}, {});
    this._emitMessage(wreq, function (err) {
      if (err) {
        this.emit('error', err);
      }
    });
...
MessageListener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  if (!msg.getName()) {
    // Ping.
    done(null, new WrappedResponse(msg, {}, null));
  } else if (this._hook) {
    // Custom hook.
    var env = {header: wreq.getHeader(), request: wreq.getRequest()};
    this._hook.call(this, msg.getName(), env, function (err, env) {
      var wres = new WrappedResponse(msg, env.header, env.error, env.response);
      done(null, wres);
    });
  } else {
    var handler = this._server.getHandlers()[msg.getName()];
    if (!handler) {
      // The underlying protocol hasn't implemented a handler for this message.
      done(new Error(f('unhandled message: %s', msg.getName())));
    } else if (msg.isOneWay()) {
      handler.call(this, wreq.getRequest());
      done(null, new WrappedResponse(msg, {}));
    } else {
      try {
        handler.call(this, wreq.getRequest(), function (err, res) {
          if (!self._server._strict) {
            // Allow `Error` objects or `null` to be passed in as argument,
            // even though these are likely not valid Avro types (since these
            // are idiomatic JavaScript).
            if (isError(err)) {
              var errType = msg.getErrorType();
              err = errType.clone(err.message, {wrapUnions: true});
            } else if (err === null) {
          ...
          done(null, new WrappedResponse(msg, {}, err, res));
        });
      } catch (err) {
...
/** Error used for representing remote RPC errors. */
function RemoteError(message, cause) {
...
/** Error used to give enhanced error messages in clients. */
function ValidationError(type) {
  Error.call(this);
  this.message = f('invalid %s', type);
  this.values = [];
}
...
function encodeError(err, header) {
  header = header || {};
  header.stackTrace = err.stack; // TODO>
