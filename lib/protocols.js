...
// TODO: Timeout for emitter options?
...
// TODO: Implement noWait for emitters' destroy.
// TODO: Rename Emitter to MessageEmitter and Listener to MessageListener.
...
var HANDSHAKE_REQUEST_TYPE = schemas.createType({
  ...
  type: 'record',
  fields: [
    {name: 'emitterHash', type: {name: 'MD5', type: 'fixed', size: 16}},
    {name: 'emitterProtocol', type: ['null', 'string'], 'default': null},
 ...
 *
 * A server can be listening to multiple emitters at a given time. This can be a
 * mix of stateful or stateless emitters (each will have its corresponding
 * listener, see below).
 *
 * For performance reasons (and simplicity), a server doesn't directly keep
 * track of its attached listeners. The downside of this is that listeners can't
 * be destroyed until they next receive a request. However, listeners keep a
 * reference to the server (via the `_call` event), which isn't removed since
 * it is assumed that listeners have no use outside of their server, so there
 * are no cases where the server would be GC'ed but not its listeners.
 *
 */
function Protocol(name, messages, types) {
  this._name = name;
  ...
  this._emitterResolvers = {};
  this._serverResolvers = {};
}

Protocol.prototype.onMessage = function (name, opts, handler) {
...
Protocol.prototype.copy = function () {
  return new Protocol(this._name, this._messages, this._types);
};

Protocol.prototype.clearCache = function () {
  // Recall that already instantiated emitters and listeners will keep a
 ...
 * Base emitter class.
 *
 * A emitter is a message emitter. See below for the two available variants.
 *
 */
...
Emitter.prototype._generateResolvers = function (hashString, serverPtcl) {
  var resolvers = {};
...
Emitter.prototype._createHandshakeRequest = function (hashString, noPtcl) {
...
Emitter.prototype._finalizeHandshake = function (tap, handshakeReq) {
  var res = HANDSHAKE_RESPONSE_TYPE._read(tap);
  this.emit('handshake', handshakeReq, res);

  if (handshakeReq.emitterProtocol && res.match === 'NONE') {
  ...
  if (res.serverHash && res.serverProtocol) {
    ...
    if (!canResolve(this, hashString)) {
      this._generateResolvers(
        hashString,
        parseProtocol(res.serverProtocol.string, this._ptclHook)
      );
    }
...
Emitter.prototype._encodeRequest = function (tap, message, req) {
  safeWrite(tap, STRING_TYPE, message.name);
  safeWrite(tap, message.requestType, req);
};

Emitter.prototype._decodeArguments = function (tap, hashString, message) {
...
StatelessEmitter.prototype.emitMessage = function (name, req, cb) {
  ...
  function emit(retry) {
    ...
    try {
      safeWrite(tap, self._idType, 0); // Unused for stateless emitters.
  ...
  function done(err, res) {
    ...
    asyncAvroCb(cb, err, res);
  }
};

StatelessEmitter.prototype.destroy = function () {
...
StatefulEmitter.prototype._noPending = function () {
  // Not worth caching redundant state for this, only called on destroy.
  return !Object.keys(this._pending).length;
};

StatefulEmitter.prototype.emitMessage = function (name, req, cb) {
  if (this._destroyed) {
    asyncAvroCb(cb, 'emitter destroyed');
    return;
  }
  ...
  var message = this._ptcl._messages[name];
  if (!message) {
    asyncAvroCb(cb, f('unknown message: %s', name));
    return;
  }
...
StatefulEmitter.prototype.destroy = function () {
  this._destroyed = true;
  if (!this._started) {
...
Listener.prototype._generateResolvers = function (hashString, emitterPtcl) {
  var resolvers = {};
  var emitterMessages = emitterPtcl._messages;
...
Listener.prototype._validateHandshake = function (reqTap, resTap) {
  ...
  if (handshakeReq) {
    this._emitterHashString = handshakeReq.emitterHash.toString('binary');
    if (!canResolve(this, this._emitterHashString)) {
      var emitterPtclString = handshakeReq.emitterProtocol;
      if (emitterPtclString) {
        try {
          this._generateResolvers(
            this._emitterHashString,
            parseProtocol(emitterPtclString.string, this._ptclHook)
          );
        } catch (err) {
          validationErr = err;
        }
      } else {
        validationErr = new Error('unknown emitter protocol hash');
      }
    }
  }
...
Listener.prototype._decodeRequest = function (tap, message) {
...
Listener.prototype._encodeSystemError = function (tap, err) {
  safeWrite(tap, BOOLEAN_TYPE, true);
  safeWrite(tap, SYSTEM_ERROR_TYPE, avroError(err));
};

Listener.prototype._encodeArguments = function (tap, message, err, res) {
 ...
 *
 */
function StatefulListener(ptcl, readable, writable) {
  Listener.call(this, ptcl);
  ...
  function onRequestData(buf) {
    ...
    if (self._destroyed) {
      onResponse({string: 'listener destroyed'});
      return;
    }
 ...
 *
 */
function asyncAvroCb(cb, err, res) {
  process.nextTick(function () { cb(avroError(err), res); });
}
 ...
 *
 */
function parseProtocol(ptclString, hook) {
 ...
 *
 * @param emitter {Emitter|Listener}
