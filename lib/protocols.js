...
function createProtocol(attrs, opts) {
  ...
  var errorType = opts.systemErrorType;
  if (!errorType) {
    // We provide a custom default error type to allow passing errors to
    // callbacks as is the norm. The only requirement is that its binary
    // serialization matches that of a string (to be able to communicate with
    // non-JavaScript implementations).
    errorType = types.createType({
      name: 'SystemError',
      type: 'error',
