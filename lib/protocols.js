...
function createProtocol(attrs, opts) {
  ...
  var errorType = opts.systemErrorType || 'string';
  if (attrs.types) {
  ...
  if (attrs.messages) {
    Object.keys(attrs.messages).forEach(function (key) {
      messages[key] = new Message(key, attrs.messages[key], opts, errorType);
    });
  }
  ...
  return new Protocol(
    name,
    messages,
    types.createType([errorType], {wrapUnions: opts.wrapUnions}),
    opts.registry || {}
  );
}
 ...
 *
 */
function Protocol(name, messages, systemErrorType, types, ptcl) {
  this._name = name;
  ...
  this._onListenerCall = function (name, req, cb) {
    ...
    if (handler) {
      ...
      cb(self._toSystemError(f('unhandled message: %s', name)));
    }
  };
...
Protocol.prototype._toSystemError = function (err) {
  ...
  var buf = Buffer.concat([new Buffer([0]), STRING_TYPE.toBuffer(err)]);
  return this._systemErrorType.fromBuffer(buf);
};
...
Protocol.prototype.emit = function (name, req, emitter, cb) {
  cb = cb || emitter._onUnhandledError;

  if (emitter._ptcl._hashString !== this._hashString) {
    callAsync(this, cb, this._toSystemError('invalid emitter'));
    return;
  }

  var message = this._messages[name];
  if (!message) {
    callAsync(this, cb, this._toSystemError(f('unknown message: %s', name)));
    return;
  }
...
function MessageEmitter(ptcl, opts) {
  ...
  var self = this;
  this._onUnhandledError = function (err) {
    // We need to convert the protocol system error to an error.
    var message = JSON.parse(self._ptcl._systemErrorType.toString(err)).string;
...
StatelessEmitter.prototype._emit = function (message, req, cb) {
  ...
  if (this._destroyed) {
    callAsync(this, cb, this._ptcl._toSystemError('emitter destroyed'));
    return;
  }
  this._pending[id] = cb;
  emit(false);
...
StatefulEmitter.prototype._emit = function (message, req, cb) {
  if (this._destroyed) {
    callAsync(this._ptcl, cb, 'emitter destroyed');
    return;
  }
  ...
  try {
    ...
    callAsync(this._ptcl, cb, this._ptcl._toSystemError(err));
    return;
  }
 ...
 *
 */
function Message(name, attrs, opts, systemErrorType) {
  this._name = name;
  ...
  errors.unshift(systemErrorType || 'string');
