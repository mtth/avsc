...
Client.prototype.getEmitters = function () { return this._emitters$.slice(); };
...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  return emitter._emit(wreq, opts, function (err, wres) {
    ...
    if (err) {
      if (self._strict$) {
        err = errType.clone(err.stack || err.toString(), {wrapUnions: true});
      }
      done(err);
      return;
    }
...
MessageEmitter.prototype._emit = function (wreq, opts, cb) {
  ...
  process.nextTick(function () {
    if (wreq.getMessage().isPing()) {
      ...
      target(wreq, wres, source);
    } else {
      chainFns(self._client._fns$, self, [wreq, wres], target, source);
    }
  });
  return this._pending;

  function target(wreq, wres, prev) {
...
MessageListener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  var isDone = false;
  self._pending++;
  chainFns(this._server._fns, this, [wreq, wres], target, source);
    ...
    if (self._hook) {
      ...
      if (!handler) {
        ...
        try {
          ...
          // unnoticed.
          prev(toRpcError('INTERNAL_SERVER_ERROR', err));
        }
      }
    }
  }
  ...
  function source(err) {
    ...
    if (!err) {
      try {
        ...
        err = toRpcError('INTERNAL_SERVER_ERROR', cause);
      }
    }
    if (err) {
...
MessageListener.prototype._encodeError = function (err, header) {
  ...
  if (header) {
    ...
    } catch (err) { /* Swallow this error... */ }
  }
  return Buffer.concat([
    hdrBuf || new Buffer([0]),
    new Buffer([1, 0]), // Error flag and first union index.
    STRING_TYPE.toBuffer(err.rpcCode || err.message || err.toString())
  ]);
}
 ...
 * @param ctx {Object} Context used to call the middleware functions, target,
 * and source.
 * @param args {Array} Array of arguments. These will be used for all forward
 * middleware functions.
 * @param target {Function} End of forward phase, the arguments this function
 * passes to its callback will be used for the backward phase. The first of
 * these arguments must be an (eventual) error.
 * @param source {Function} Final handler, it must accept the backward phase
 * argumens.
 */
  ...
  function forward(pos) {
    ...
    if (pos < fns.length) {
      fns[pos].apply(ctx, args.concat(function (err, cb) {
        if (isDone) {
          source.call(ctx, new Error('duplicate middleware forward call'));
          return;
        }
        isDone = true;
        if (err) {
      ...
      target.apply(ctx, args.concat(function () {
        if (isDone) {
          source.call(ctx, new Error('duplicate handler call'));
          return;
        }
        isDone = true;
  ...
  function backward() {
    ...
    if (cb) {
      cb.apply(ctx, args.concat(function (err) {
        if (isDone) {
          source.call(ctx, new Error('duplicate middleware backward call'));
          return;
        }
        isDone = true;
        backward();
