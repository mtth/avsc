...
function Emitter(ptcl, opts) {
  ...
  this._adapter = this._cache[serverPtcl._hs] || new Adapter(serverPtcl);
...
Emitter.prototype._getAdapter = function (hres) {
  ...
  if (serverBuf.equals(clientBuf)) {
    adapter = new Adapter(this._ptcl);
  } else {
    adapter = this._cache[serverBuf];
    if (adapter) {
      return adapter;
    }
    var serverPtcl = createProtocol(JSON.parse(hres.serverProtocol));
    adapter = new Adapter(this._ptcl, serverPtcl);
  }
...
Listener.prototype._getAdapter = function (hreq) {
  ...
  if (clientBuf.equals(serverBuf)) {
    ...
    adapter = new Adapter(clientPtcl, this._ptcl);
  }
 ...
 *
 */
function Adapter(clientPtcl, serverPtcl) {
