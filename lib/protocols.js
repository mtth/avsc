 ...
 */
function Protocol(name, messages, types, doc, str, server) {
...
Protocol.forSchema = function (schema, opts) {
  ...
  return new Protocol(name, msgs, opts.registry, schema.doc, str);
};
...
Protocol.prototype.toJSON = Type.prototype.toJSON;
...
Client.prototype.getRemoteSchemas = function () {
  return Adapter._getRemoteSchemas(this._ptcl$, this._cache$, true);
};
...
Server.prototype.getRemoteSchemas = function () {
  return Adapter._getRemoteSchemas(this._ptcl, this._cache, false);
};
...
MessageEmitter.prototype._createHandshakeRequest = function (adapter, noPtcl) {
  ...
  return {
    ...
    clientProtocol: noPtcl ? null : ptcl._str,
...
MessageEmitter.prototype._getAdapter = function (hres) {
  ...
  adapter = new Adapter(this._client._ptcl$, serverPtcl, fgpt, true);
...
MessageListener.prototype._createHandshakeResponse = function (err, hreq) {
  ...
  return {
    ...
    serverProtocol: serverMatch ? null : ptcl._str,
...
MessageListener.prototype._getAdapter = function (hreq) {
  ...
  adapter = new Adapter(clientPtcl, this._server.getProtocol(), fgpt, true);
 ...
 */
function Adapter(clientPtcl, serverPtcl, fingerprint, isRemote) {
...
Adapter._populateCache = function (cache, ptcl, schemas, isClient) {
  Object.keys(schemas).forEach(function (fgpt) {
    ...
    cache[fgpt] = new Adapter(clientPtcl, serverPtcl, fgpt, true);
  });
  return cache;
};

Adapter._getRemoteSchemas = function (ptcl, cache, isClient) {
  var schemas = {};
  Object.keys(cache).forEach(function (hs) {
    var adapter = cache[hs];
    if (adapter._isRemote) {
      var ptcl = isClient ? adapter._serverPtcl : adapter._clientPtcl;
      schemas[hs] = JSON.parse(ptcl._str);
    }
  });
