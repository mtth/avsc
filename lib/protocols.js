...
Emitter.prototype.getCache = function () { return this._cache; };
...
Emitter.prototype.isDestroyed = function () { return this._destroyed; };

Emitter.prototype.emitMessage = function (name, reqEnv, opts, cb) {
  ...
  if (this._destroyed) {
    ...
    var msg = this._ptcl.getMessage(name);
    if (!msg) {
  ...
  var id = this._registry.add(timeout, function (err, resBuf, adapter) {
    if (!err) {
      try {
        var resEnv = adapter.decodeResponse(resBuf, msg.getName()).envelope;
      } catch (cause) {
        err = wrapError('invalid response', cause);
      }
    }
    cb.call(this, err, resEnv, adapter && adapter.getServerProtocol());
  ...
  var self = this;
  return self._send(id, reqBuf, msg.isOneWay());
};
...
function StatefulEmitter(ptcl, readable, writable, opts) {
  ...
  this._connected = !!(opts && opts.noHandshake);
  this._readable.on('end', function () { self.destroy(); });
  this._writable.on('finish', function () { self.destroy(true); });
...
Listener.prototype.getCache = function () { return this._cache; };

Listener.prototype.getPending = function () { return this._pending; };

Listener.prototype.getProtocol = function () { return this._ptcl; };

Listener.prototype.isDestroyed = function () { return this._destroyed; };

Listener.prototype.onMessage = function (fn) {
  this._hook = fn;
  return this;
};
...
Listener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  if (this._hook) {
    // Custom hook.
    this._hook.call(this, name, reqEnv, adapter.getClientProtocol(), done);
  } else if (handler) {
    try {
...
function StatefulListener(ptcl, readable, writable, opts) {
  ...
  this._writable
    .on('finish', function () { self.destroy(true); });
