...
Protocol.prototype.getHandler = util.deprecate(
  ...
  'use `server.getHandlers` instead'
);
...
/** Function used when names clash for example. */
...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  return this._emitter$._emit(wreq, opts, function (err, wres) {
...
Server.prototype.onMessage = function (name, handler) {
  ...
  this._handlers[name] = function (req, cb) {
    if (msg.isOneWay()) {
      handler.call(listener, req);
    } else {
      var listener = this;
      return handler.call(listener, req, function (err, res) {
        if (!listener.getServer()._strict) {
