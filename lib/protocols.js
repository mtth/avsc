 ...
 * and source.
 * @param args {Array} Array of "forward" arguments. These will be used for all
 * forward middleware functions.
 * @param target {Function} End of forward phase, the arguments this function
 * passes to its callback will be used for the backward phase.
 * @param source {Function} Final handler, also used if any errors are passed
 * in to middleware callbacks.
 */
  ...
  function forward(pos) {
    ...
    if (pos < fns.length) {
      ...
      target.apply(ctx, args.concat(function () {
        ...
        if (isDone) {
          args.push(new Error('duplicate handler call'));
        } else {
          isDone = true;
