...
// TODO: Switch from `getType` to `getTypes` method.
...
Protocol.prototype.emit = function (name, req, emitter, cb) {
  ...
  var self = this;
  emitter._emit(message, req, function (err, res) {
    ...
    if (cb) {
      ...
      emitter.emit('error', wrapError('unhandled error', err));
    }
  });
};
...
/**
 * Wrap an error inside another error.
 *
 * The original error is available as `cause` field.
 *
 */
