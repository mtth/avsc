...
function Protocol(attrs, opts) {
  ...
  // Resolver are split since we want clients to still be able to talk to
...
function Client(ptcl, opts) {
  ...
  this._serverPtclOpts = opts.serverProtocolOpts;
  this._includeProtocol = !!opts.includeProtocol;
  this._serverHashString = opts.serverHash ?
    opts.serverHash.toString('binary') :
    ptcl._hashString; // Sane default.
}
util.inherits(Client, events.EventEmitter);

Client.prototype._addResolvers = function (serverPtcl) {
  var clientPtcl = this._ptcl;
  if (serverPtcl._hashString === this._hashString) {
  ...
  clientPtcl._clientResolvers[serverPtcl._hashString] = resolvers;
};
...
Client.prototype._decodeHandshake = function (tap) {
  ...
  assert(!this._canResolve(handshake.serverHash.toString('binary')));

  var serverPtcl = new Protocol(
    JSON.parse(handshake.serverProtocol.string),
    this._serverPtclOpts
  );

  var serverHash = handshake.serverHash['org.apache.avro.ipc.MD5'];
  assert(serverPtcl.getHash().equals(serverHash));

  this._addResolvers(serverPtcl);
  this._serverHashString = serverPtcl._hashString;
      ...
      .once('data', function (buf) {
        ...
        try {
          if (!self._decodeHandshake(tap)) {
            // Handshake failed, server doesn't have client ptcl.
...
function Channel(ptcl, opts) {
  ...
  this._clientPtclOpts = opts.clientProtocolOpts;
}
util.inherits(Channel, events.EventEmitter);

Channel.prototype._addResolvers = function (clientPtcl) {
  var serverPtcl = this._ptcl;
  if (clientPtcl._hashString === serverPtcl._hashString) {
    return; // Protocols are equal, no need to store any resolvers.
  }
  ...
  serverPtcl._serverResolvers[clientPtcl._hashString] = resolvers;
};
...
Channel.prototype._decodeHandshake = function (tap) {
  ...
  this._clientHashString = handshake.clientHash.toString('binary');

  var serverPtcl = this._ptcl;
  var match = handshake.serverHash.equals(serverPtcl.getHash());
  ...
  var clientPtcl = new Protocol(
    JSON.parse(handshake.clientProtocol.string),
    this._clientPtclOpts
  );
  this._addResolvers(clientPtcl);
  return match ? 'BOTH' : 'CLIENT';
};
