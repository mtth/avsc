...
// TODO: Decouple stateful and stateless from stream/factory for emitters and
// listeners. Perhaps having a Stream (always stateful), Stateful, and
// Stateless of each.
 ...
 * listeners. Note that a protocol can be listening to multiple emitters at a
 * given time. This can be a mix of stateful or stateless emitters (each will
 * have its corresponding listener, see below).
 *
 */
...
Protocol.prototype.emit = function (name, req, emitter, cb) {
  if (
    ...
    asyncAvroCb(this, cb, new Error('invalid emitter'));
    return;
  }
  emitter._emit(name, req, cb);
};
...
StatelessEmitter.prototype._emit = function (name, req, cb) {
  ...
  function done(err, res) {
    ...
    asyncAvroCb(self._ptcl, cb, err, res);
    if (self._destroyed) {
      // Check whether this was the last pending callback, but allow the
      // previous callback to go through first.
...
StatefulEmitter.prototype._emit = function (name, req, cb) {
  if (this._destroyed) {
  ...
  var self = this;
  if (!this._started) {
    this.once('_start', function () { self._emit(name, req, cb); });
    return;
  }
  ...
  var message = this._ptcl._messages[name];
  if (!message) {
    asyncAvroCb(this._ptcl, cb, f('unknown message: %s', name));
    return;
  }
...
function StatelessListener(ptcl, readableFactory, opts) {
  MessageListener.call(this, ptcl);
...
function StatefulListener(ptcl, readable, writable) {
  ...
  function onRequestData(buf) {
    ...
    function onResponse(err, res) {
      ...
      self._pending--;
      if (self._finished) {
        // We won't be able to send a reply back, exit early.
        return;
      }
...
function avroError(err) {
  if (err instanceof Error) {
    ...
    err = err.stack || /* istanbul ignore next */ '' + err;
  }
