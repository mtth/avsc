...
function MessageEmitter(ptcl, opts) {
  ...
  this._prefix = normalizePrefix(opts.prefix);
...
function normalizePrefix(prefix) {
  if (prefix === undefined) {
    var buf = new Buffer(PREFIX_LENGTH);
    buf.fill(0);
    return buf;
  }
  return utils.getHash(prefix).readInt16BE() << (32 - PREFIX_LENGTH);
}
