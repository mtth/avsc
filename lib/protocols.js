...
// TODO: Implement noWait for clients' destroy.
// TODO: Rename Client to MessageEmitter and Channel to MessageListener.
...
Protocol.prototype.createChannel = function (transport, opts) {
  var channel;
...
StatefulClient.prototype.emitMessage = function (name, req, cb) {
  if (this._destroyed) {
    asyncAvroCb(cb, 'client destroyed');
    return;
  }
      ...
      if (clientPtclString) {
        try {
          this._generateResolvers(
            ...
            parseProtocol(clientPtclString.string, this._ptclHook)
          );
        } catch (err) {
          validationErr = err;
        }
