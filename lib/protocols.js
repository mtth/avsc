...
// etc.)?
// TODO: Split MessageDecoder and MessageEncoder from MessageEmitter and
...
StatelessEmitter.prototype._emit = function (message, req, cb) {
  if (this._destroyed) {
    process.nextTick(cb, this._ptcl._toSystemError('emitter destroyed'));
    return;
  }
...
StatefulEmitter.prototype._emit = function (message, req, cb) {
  ...
  function emit() {
    ...
    try {
      ...
      self._popCall(id);
      process.nextTick(cb, self._ptcl._toSystemError(err));
      return;
    }
