...
Client.prototype._addResolvers = function (serverPtcl, serverHashString) {
  if (serverHashString === this._ptcl._hashString) {
    return;
  }
  ...
  this._ptcl._clientResolvers[serverHashString] = resolvers;
};
...
Client.prototype._decodeHandshake = function (tap) {
  ...
  this._addResolvers(serverPtcl, this._serverHashString);
...
StatefulClient.prototype.emitMessage = function (name, params, cb) {
  ...
  var self = this;
  if (!this._started) {
    this.once('_start', function () { self.emitMessage(name, params, cb); });
    return;
  }
...
StatefulClient.prototype.destroy = function () {
  this._destroyed = true;
  if (!this._started) {
    this.emit('_start'); // Error out all pending calls.
  }
  if (this._noPending()) {
    this.emit('end');
  }
...
Server.prototype.addTransport = function (transport, opts) {
  ...
  var self = this;
  channel
    .on('_call', function (name, params, cb) {
...
Channel.prototype._addResolvers = function (clientPtcl, clientHashString) {
  if (clientHashString === this._ptcl._hashString) {
    return;
  }
  ...
  this._ptcl._serverResolvers[clientPtcl._hashString] = resolvers;
};
...
Channel.prototype._decodeHandshake = function (tap) {
  ...
  this._addResolvers(clientPtcl, this._clientHashString);
  return match ? 'BOTH' : 'CLIENT';
};
