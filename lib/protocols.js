...
StatelessEmitter.prototype.destroy = function (noWait) {
  ...
  if (noWait) {
    ...
    pendingIds.forEach(function (id) {
      this._pending[id](this._ptcl._toSystemError('interrupted'));
...
function StatefulEmitter(ptcl, readable, writable, opts) {
  ...
  function onMessageData(buf) {
    ...
    try {
      ...
      info.cb(self._ptcl._toSystemError(err));
      return;
    }
    delete self._pending[id];
...
StatefulEmitter.prototype.destroy = function (noWait) {
  ...
  pendingIds.forEach(function (id) {
    ...
    delete this._pending[id];
    cb(this._ptcl._toSystemError('interrupted'));
  }, this);
...
/**
 * Convert as much information as possible from an error into a string.
 *
 * @param err {Error} Some error.
 *
 * Stack frame information is not cross-browser unfortunately so this is an
 * approximation.
 *
 */
function errorToString(err) {
  return err.stacktrace || err.stack || /* istanbul ignore next */ err.message;
}
