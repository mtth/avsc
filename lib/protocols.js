...
Protocol.getFingerprint = function (type, algorithm) {
  // This method isn't included on the type directly to allow the lightest
  // browserify distribution to not include an implementation.
  return hash(type.getSchema(), algorithm);
};

Protocol.prototype.getSchema = function (noDeref) {
  var namedTypes = [];
...
Protocol.prototype.getFingerprint = function (algorithm) {
  return hash(this.getSchema(), algorithm);
};
...
MessageEmitter.prototype._createHandshakeRequest = function (
  ...
  return new HandshakeRequest(
    getHash(this._ptcl),
    noPtcl ? null : {string: this._ptcl.getSchema()},
...
MessageListener.prototype._validateHandshake = function (reqTap, resTap) {
  ...
  var handshakeRes = new HandshakeResponse(
    ...
    serverMatch ? null : {string: this._ptcl.getSchema()},
...
function hash(str, algorithm) {
  ...
  var stream = crypto.createHash(algorithm);
  stream.end(str);
  return stream.read();
}

/**
 * Convenience function to get a protocol's hash as a buffer.
 *
 * This is more efficient than recomputing it each time.
 *
