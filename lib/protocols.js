 ...
 */
function Protocol(name, messages, types, handlers, doc) {
  if (types === undefined) {
...
Protocol.forSchema = function (schema, opts) {
  ...
  return new Protocol(name, messages, opts.registry || {}, null, schema.doc);
};
...
function discoverProtocolSchema(transport, opts, cb) {
  ...
  var emitter = Protocol.forSchema({protocol: 'Empty'}).createEmitter(transport, {
      ...
      try {
        ...
        resBuf = Buffer.concat([
          ...
          noError ?
            ...
            serverMsg.getErrorType().toBuffer(resErr)
        ]);
      } catch (cause) {
        err = wrapError('invalid response', cause);
      }
...
function Message(name, schema, opts) {
  ...
  this._requestType = Type.forSchema({
    name: recordName,
    type: 'record',
  ...
  this._responseType = Type.forSchema(schema.response, opts);

  var errors = schema.errors || [];
  errors.unshift('string');
  this._errorType = Type.forSchema(errors, opts);
