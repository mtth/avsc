...
function createProtocol(attrs, opts) {
  ...
  if (attrs.types) {
    parseProtocolTypes(attrs.types, opts);
  }
...
/**
 * Register an array of types.
 *
 * Type declarations in this function will be "hoisted" to the top, allowing
 * for circular references without nested declarations (useful to support all
 * schemas in spite of IDL limitations).
 *
 */
function parseProtocolTypes(ptclTypes, opts) {
  var i = 0;
  var l = ptclTypes.length;
  var typeHook = opts.typeHook; // Original type hook.
  var indices = {}; // Indices by reference.
  opts.typeHook = function (attrs, opts) {
    if (
      typeof attrs == 'string' &&
      opts.registry[attrs] === undefined &&
      !types.isPrimitive(attrs)
    ) {
      // Unknown reference.
      var j = indices[attrs];
      if (j === undefined) {
        // We keep track of which index an unknown reference was first seen at
        // to support backtracking in case a logical type can't be applied.
        indices[attrs] = i;
      } else {
        i = j;
      }
      if (i < l) {
        types.createType(ptclTypes[i++], opts);
      }
    }
    if (typeHook) {
      return typeHook(attrs, opts);
    }
  };
  while (i < l) {
    types.createType(ptclTypes[i++], opts);
...
function MessageEmitter(ptcl, opts) {
  ...
  this._endWritable = !!utils.getOption(opts, 'endWritable', !opts.scope);
...
MessageEmitter.prototype.emitMessage = function (name, reqEnv, opts, cb) {
  ...
  return this._send(id | this._prefix, reqBuf, !!msg && msg.isOneWay());
};
...
/**
 * Compute a prefix of fixed length from a buffer or string.
 *
 */
function normalizedPrefix(scope) {
  return scope ?
    utils.getHash(scope).readInt16BE() << (32 - PREFIX_LENGTH) :
    0;
}

/**
 * Check whether a message's ID matches the prefix.
 *
 */
