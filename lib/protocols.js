...
function Protocol(name, messages, types, doc, server) {
  ...
  this._server = server || this.createServer({strictErrors: true});
}
...
Protocol.prototype.createClient = function (opts, transport) {
...
Protocol.prototype.subprotocol = util.deprecate(
  function () {
    ...
    server._handlers = Object.create(this._server._handlers);
    return new Protocol(
      this._name,
      this._messages,
      this._types,
  ...
  'use `protocol.createServer` and `server.onMessage` instead'
);
...
Protocol.prototype.createListener = util.deprecate(
  function (transport, opts) {
    ...
    server._handlers = this._server._handlers;
...
Protocol.prototype.on = util.deprecate(
  function (name, handler) {
    ...
    this._server.onMessage(name, function (req, cb) {
      var msg = self.getMessage(name);
      if (msg.isOneWay()) {
        return handler.call(self, req, listener);
      } else {
        var listener = this;
        return handler.call(self, req, listener, function (err, res) {
          if (!listener.getServer()._strict) {
            err = coerceError(err, msg.getErrorType());
          }
          cb(err, res);
        });
      }
...
Client.prototype.getActiveEmitters = function () {
  ...
  this._handlers[name] = (msg.isOneWay() || this._strict) ?
    ...
    function (req, cb) {
      ...
      return handler.call(listener, req, function (err, res) {
        ...
        cb(coerceError(err, msg.getErrorType()), res);
      });
    };
  return this;
};
...
MessageListener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  if (!msg.getName()) {
    ...
    var handler = this._server._handlers[msg.getName()];
    if (!handler) {
...
/**
 * Transform an error to be a valid Avro type.
 *
 */
function coerceError(err, errType) {
  if (isError(err)) {
    err = errType.clone(err.message, {wrapUnions: true});
  } else if (err === null) {
    err = undefined;
