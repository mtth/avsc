...
let POOL = new BufferPool(4096);

// Valid (field, type, and symbol) name regex.
let NAME_PATTERN = /^[A-Za-z_][A-Za-z0-9_]*$/;
...
Tap.prototype.readLong = function () {
  let n = 0;
  let k = 0;
  let buf = this.buf;
  let b, h, f, fk;
...
Tap.prototype.skipLong = function () {
  let buf = this.buf;
  while (buf[this.pos++] & 0x80) {}
};

Tap.prototype.writeLong = function (n) {
  let buf = this.buf;
  let f, m;
...
Tap.prototype.matchLong = function (tap) {
