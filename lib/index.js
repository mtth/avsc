...
/** Writable stream of records that appends to a local Avro file. */
function createFileAppender(path, schema, opts) {
  var fd, existingFile, header, encoder, filestream;
  
  opts = opts || {};
  
  try {
    existingFile = fs.statSync(path);
    if (existingFile && existingFile.size > 0) {
      header = extractFileHeader(path);
      if (header) {
        opts.omitHeader = true;
        opts.syncMarker = header.sync;
      } else {
        throw 'Can`t read avro header ' + path + '. Is it a valid avro file?';
      }
    } 
  } catch (e) {
  ...
  filestream = fs.createWriteStream(null, {defaultEncoding: 'binary', fd: fd});
  encoder = new containers.streams.BlockEncoder(schema, opts);
