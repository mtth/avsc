...
// TODO: Logical type support for `combine`.
// TODO: Customizable support for defaults in `combine` (e.g. override order?).
...
// Placeholder type used for empty arrays.
...
function infer(val, opts) {
  if (opts && opts.valueHook) {
    var type = opts.valueHook(val);
    if (type !== undefined) {
