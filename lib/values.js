...
var EMPTY_ARRAY_TYPE = Type.create({type: 'array', items: 'null'});
...
function infer(val, opts) {
  ...
  switch (typeof val) {
    case 'string':
      return Type.create('string', opts);
    case 'boolean':
      return Type.create('boolean', opts);
    case 'number':
      if ((val | 0) === val) {
        return Type.create('int', opts);
      } else if (Math.abs(val) < 9007199254740991) {
        return Type.create('float', opts);
      }
      return Type.create('double', opts);
    case 'object':
      if (val === null) {
        return Type.create('null', opts);
...
function combine(types, opts) {
  ...
  if (numWrappedUnions) {
    ...
    try {
      unionType = Type.create(Object.keys(branchTypes).map(function (name) {
  ...
  if (augmented.length === 1) {
    ...
    return Type.create(augmented, opts);
  }
}
...
function combineStrings(types, opts) {
  ...
  return Type.create({type: 'enum', symbols: Object.keys(symbols)}, opts);
}
...
function combineBuffers(types, opts) {
  ...
  return size < 0 ? Type.create('bytes', opts) : types[0];
}
...
function combineObjects(types, opts) {
    ...
    for (i = 0, l = fieldNames.length; i < l; i++) {
      ...
      if (
        ...
        if (opts && opts.strictDefaults) {
          ...
          fieldTypes[fieldName].unshift(Type.create('null', opts));
      ...
      fields: fieldNames.map(function (s) {
        ...
        if (
          ...
          if (i > 0) {
            ...
            fieldType = Type.create(unionTypes, opts);
          }
        }
        return {
          name: s,
  ...
  return Type.create(schema, opts);
}
