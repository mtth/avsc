...
// TODO: Add a few built-in logical types (e.g. dates).
...
// Placeholder type used for empty arrays. It is detected when combining types.
...
function combine(types, opts) {
  ...
  types.forEach(function (type) {
    switch (type.getTypeName()) {
      case 'union:wrapped':
        throw new Error('wrapped unions cannot be augmented');
      case 'union:unwrapped':
        expanded.concat(type.getTypes());
        break;
      default:
        expanded.push(type);
    }
...
function combineObjects(types, opts) {
    ...
    if (type.getTypeName() === 'map') {
      ...
      for (j = 0, m = fields.length; j < m; j++) {
        ...
        if (fieldDefault !== undefined) {
          // TODO: Throw if overriding a default?
  ...
  for (i = 0, l = fieldNames.length; i < l; i++) {
    ...
    if (fieldTypes[fieldName].length < types.length) {
      // At least one of the records is missing this field.
      if (opts && opts.noNullDefaults) {
        isValidRecord = false;
      } else {
        fieldTypes.push(createType('null', opts));
  ...
  if (isValidRecord) {
    attrs = {
      ...
      fields: fieldNames.map(function (s) {
        return {
          name: s,
          type: combine(fieldTypes[s], opts),
