...
function assembleProtocol(fpath, opts, cb) {
  ...
  if (!opts.relativeResolveHook) {
    opts.relativeResolveHook = files.createRelativeResolveHook();
  }
  ...
  function fetchImports(protocol, imports, fpath, cb) {
    ...
    function next() {
      ...
      let importPath = opts.relativeResolveHook(fpath, info.name);
      if (info.kind === 'idl') {
...
class Reader {
  ...
  static readSchema (str, opts) {
    ...
    let schema = reader._readType(doc === undefined ? {} : {doc: doc}, true);
  ...
  _readMap (schema) {
    ...
    let implicitTags = tk.next({val: '<', silent: silent}) === undefined;
