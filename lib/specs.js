...
/**
 * IDL to protocol (service definition) and schema (type definition) parsing
 * logic.
 *
 */
 ...
 *
 */
function assembleSpec(fpath, opts, cb) {
  ...
  function fetchImports() {
    ...
    if (!info) {
      ...
      if (info.kind === 'idl') {
        assembleSpec(importPath, opts, mergeImportedSchema);
      } else {
...
function parse(str) {
  ...
  if (typeof str == 'string' && ~str.indexOf('/') && files.existsSync(str)) {
    ...
    try {
      ...
      assembleSpec(str, opts, function (err, protocolSchema) {
  ...
  if (typeof schema != 'string' || schema === 'null') {
    // This last predicate is to allow `parse('null')` to work similarly
    // to `parse('int')` and other primitives (null needs to be handled
  ...
  try {
    ...
    try {
      return parseServiceSpec(schema);
    } catch (err) {
      try {
        return parseTypeSpec(schema);
      } catch (err) {
        return schema;
      }
    }
...
function parseServiceSpec(str, opts) {
  var protocol = _readProtocol(str, opts);
...
module.exports = {
  ...
  parse: parse,
  parseServiceSpec: parseServiceSpec,
