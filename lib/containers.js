...
// Type of Avro header.
var HEADER_TYPE = types.Type.create({
...
// Type of each block.
var BLOCK_TYPE = types.Type.create({
  namespace: 'org.apache.avro.file',
  name: 'Block',
  type: 'record',
  fields : [
...
var LONG_TYPE = types.Type.create('long');
...
function RawDecoder(schema, opts) {
  ...
  this._type = types.Type.create(schema);
...
BlockEncoder.prototype._write = function (val, encoding, cb) {
  ...
  if (!this._omitHeader) {
    var schemaStr = this._schema ?
      JSON.stringify(this._schema) :
      this._type.getSchema({asString: true});
    var meta = {
