...
BlockDecoder.prototype._createBlockCallback = function (cb) {
  ...
  return function (cause, data) {
    if (cause) {
      var err = new Error('decompression error');
      err.cause = cause;
      self.emit('error', err);
      cb();
    } else {
...
function BlockEncoder(schema, opts) {
  var highWaterMark = undefined;

  opts = opts || {};

  this._writeFlushed = opts.writeFlushed || false;
  if (this._writeFlushed) {
    highWaterMark = 1;
  }
...
BlockEncoder.prototype._write = function (val, encoding, cb) {
  ...
  if (this._writeFlushed) {
    this._write = this._writeChunkFlushed;
  } else {
    this._write = this._writeChunk;
  }
...
BlockEncoder.prototype._writeChunkFlushed = function (val, encoding, cb) {
  var tap = this._tap;
  var tapValid = false;
  ...
  do {
    ...
    tap.pos = 0;
    this._writeValue(tap, val);
    tapValid = tap.isValid();
    if(!tapValid) {
      this._blockSize = tap.pos;
    }
  } while (!tapValid)

  this._flushChunk(tap.pos, cb);
}
...
BlockEncoder.prototype._createBlockCallback = function (cb) {
  ...
  return function (cause, data) {
    if (cause) {
      var err = new Error('compression error');
      err.cause = cause;
      self.emit('error', err);
      return;
    }
