...
RecordType.prototype._createChecker = function () {
  ...
  if (!this.fields.length) {
    ...
    for (i = 0, l = this.fields.length; i < l; i++) {
      ...
      body += '    b &= ';
      if (field.defaultValue() === undefined) {
        body += 't' + i + '._check((v.' + field.name + (field.aliases != null && field.aliases.length > 0 ? ' || v.' + field.aliases.join(' || v.') : '') + '), f, h, p);\n';
      } else {
        body += 'v' + i + ' === undefined || ';
    ...
    body += this.fields.map(function (field, i) {
      return field.defaultValue() === undefined ?
        't' + i + '._check((v.' + field.name + (field.aliases != null && field.aliases.length > 0 ? ' || v.' + field.aliases.join(' || v.') : '') + '), f)' :
...
RecordType.prototype._createWriter = function () {
  ...
  for (i = 0, l = this.fields.length; i < l; i++) {
    ...
    body += '  ';
    if (field.defaultValue() === undefined) {
      body += 't' + i + '._write(t, (v.' + field.name + (field.aliases != null && field.aliases.length > 0 ? ' || v.' + field.aliases.join(' || v.') : '') + '));\n';
    } else {
