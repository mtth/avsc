...
function Field(schema, opts) {
  ...
  if (value !== undefined) {
    var type = this.type;
    var obj = type.clone(value, {coerce: true});
    // Clone once first to handle all the coercions once.
    if (type.type === 'union' && !type.types[0].isValid(obj)) {
      // Avro forces the default value to be of the first type in the enum, so
      // we must do a bit of extra logic here.
      throw new AvscError('invalid %s default: %j', this.name, value);
    }
    this.getDefault = function () { return type.clone(obj); };
  }
}

Field.prototype.getDefault = function () { return undefined; };
