...
function Type(schema, opts) {
  ...
  this._doc = (schema && schema.doc) ? '' + schema.doc : undefined;
...
Type.prototype.getDocumentation = function () { return this._doc; };

Type.prototype.getName = function (asBranch) {
  if (this._name || !asBranch) {
    return this._name;
  }
  var type = Type.isType(this, 'logical') ? this._underlyingType : this;
  return Type.isType(type, 'union') ? undefined : type.getTypeName();
};

Type.prototype.getSchema = function (opts) {
  // Copy the options to avoid mutating the original options object.
  return this._attrs({
...
Type.prototype.toJSON = function () {
  // Convenience to allow using `JSON.stringify(type)` to get a type's
  // canonical schema. Note that we don't forward any arguments.
  return this._attrs();
};
...
Type.prototype.getFingerprint = function (algorithm) {
  if (!algorithm) {
    if (!this._hs) {
      var schemaStr = JSON.stringify(this.getSchema());
      this._hs = utils.getHash(schemaStr).toString('binary');
    }
    return new Buffer(this._hs, 'binary');
  } else {
    return utils.getHash(JSON.stringify(this.getSchema()), algorithm);
  }
};
...
Type.prototype.inspect = function () {
  ...
  if (isPrimitive(typeName)) {
    ...
    var obj = this.getSchema({exportAttrs: true, noDeref: true});
...
Type.prototype._attrs = function (opts) {
  ...
  if (opts.exportAttrs) {
    if (this._aliases && this._aliases.length) {
      schema.aliases = this._aliases;
    }
...
Type.prototype.getAliases = utils.abstractFunction;
...
PrimitiveType.prototype._deref = function () { return this.getTypeName(); };
...
function UnionType(schema, opts) {
  ...
  this._types = schema.map(function (obj) { return Type.forSchema(obj, opts); });
...
UnionType.prototype._deref = function (schema, opts) {
  return this._types.map(function (t) { return t._attrs(opts); });
};

UnionType.prototype.getTypes = function () { return this._types; };
...
EnumType.prototype._deref = function (schema) {
  schema.symbols = this._symbols;
};

EnumType.prototype.getAliases = function () { return this._aliases; };

EnumType.prototype.getSymbols = function () { return this._symbols; };
...
FixedType.prototype._deref = function (schema) { schema.size = this._size; };
...
function MapType(schema, opts) {
  ...
  this._values = Type.forSchema(schema.values, opts);
}
...
MapType.prototype._deref = function (schema, opts) {
  schema.values = this._values._attrs(opts);
};
...
function ArrayType(schema, opts) {
  ...
  this._items = Type.forSchema(schema.items, opts);
}
...
ArrayType.prototype._deref = function (schema, opts) {
  schema.items = this._items._attrs(opts);
};
...
RecordType.prototype._deref = function (schema, opts) {
  schema.fields = this._fields.map(function (field) {
...
RecordType.prototype.getFields = function () { return this._fields; };
...
LogicalType.prototype._deref = function (schema, opts) {
  ...
  var isVisited = type.getName() !== undefined && opts.derefed[type.getName()];
...
function Field(schema, opts) {
  ...
  this._name = name;
  this._type = Type.forSchema(schema.type, opts);
  this._aliases = schema.aliases || [];
  this._doc = schema.doc !== undefined ? '' + schema.doc : undefined;
...
Field.prototype.getDocumentation = function () { return this._doc; };
    ...
    schema = {
      ...
      fields: fieldNames.map(function (s) {
        ...
        if (
          ...
          var unionTypes = fieldType.getTypes().slice();
          var i, l;
