...
UnionType.prototype._createResolver = function (type, opts) {
  ...
  for (i = 0, l = this._types.length; i < l; i++) {
    try {
      resolver = this._types[i].createResolver(type, opts);
    } catch (err) {
      continue;
    }
  ...
  if (opts && opts.wrapUnions) {
    ...
    if (typeof obj == 'object') {
      ...
      if (keys.length === 1) {
        ...
        if (i !== undefined) {
          return new this._constructors[i](this._types[i].clone(obj[name], opts));
        }
      }
    }
...
FixedType.prototype.getSymbols = function () { return this._symbols.slice(); };
...
MapType.prototype._read = function (tap) {
  return tap.readMap(this._values._read, this._values);
};

MapType.prototype._skip = function (tap) {
  return tap.skipMap(this._values._skip, this._values);
};

MapType.prototype._write = function (tap, obj) {
  return tap.writeMap(obj, this._values._write, this._values);
};
...
MapType.prototype.clone = function (obj, opts) {
  if (obj && typeof obj == 'object' && !(obj instanceof Array)) {
    ...
    for (i = 0, l = keys.length; i < l; i++) {
      key = keys[i];
      copy[key] = this._values.clone(obj[key], opts);
    }
    return copy;
  }
...
MapType.prototype._createResolver = function (type, opts) {
  if (type instanceof MapType) {
    var resolver = this._values.createResolver(type._values, opts);
...
ArrayType.prototype._read = function (tap) {
  return tap.readArray(this._items._read, this._items);
};
...
ArrayType.prototype._createResolver = function (type, opts) {
  if (type instanceof ArrayType) {
    var resolver = this._items.createResolver(type._items, opts);
