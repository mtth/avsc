...
/**
 * Returns a copy of the default registry. Deprecated.
 *
 * This function was required before `fromSchema` lazily instantiated primitive
 * types. Starting from `2.1.1`, primitive types are added when needed so there
 * is no need to prime the registry anymore.
 *
 ...
 *
 */
Type.getDefaultRegistry = util.deprecate(function () {
  var registry = {};
  ...
  return registry;
}, 'calls to getDefaultRegistry are redundant, use an empty object instead');
...
Type.fromSchema = function (schema, opts) {
  ...
  if (typeof schema == 'string') { // Type reference.
    ...
    if (!type && isPrimitive(schema)) {
      // Reference to a primitive type. These are also defined names so we
      // return it and add it to the registry for future reference.
      return opts.registry[schema] = new TYPES[schema]();
    } else if (type) {
      return type;
    }
...
Type.prototype.fromString = function (str) {
  return this.clone(JSON.parse(str), {coerceBuffers: true, roundFloats: true});
};
 ...
 * Floats.
 *
 * These are slightly tricky as well since JavaScript number literals are
 * always doubles. To ensure the data is never modified during (binary)
 * serialization, we explicitly require 32 bit compatibility.
 *
 */
...
FloatType.prototype._updateResolver = function (resolver, type) {
  if (type instanceof FloatType || type instanceof IntType) {
    ...
    // We might have to round the number down to single precision.
    resolver._read = function (tap) { return Math.fround(type._read(tap)); };
  }
};
FloatType.prototype.clone = function (obj, opts) {
  if (typeof obj == 'number' && opts && opts.roundFloats) {
    obj = Math.fround(obj);
  }
  checkIsValid(this, obj);
  return obj;
};
FloatType.prototype.isValid = function (o) { return o === Math.fround(o); };
