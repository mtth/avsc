...
// TODO: Move `getSchema` implementation off of `JSON.stringify` (and remove
// `asString` option as a result).
...
// Variable used to decide whether to include documentation, aliases, field
// order and defaults, and logical attributes when getting a type's schema. A
...
Type.prototype.getSchema = function (opts) {
  var str = stringify(this, opts);
  return (opts && opts.asString) ? str : JSON.parse(str);
};
...
Type.prototype.getFingerprint = function (algorithm) {
  if (!algorithm) {
    if (!this._hs) {
      var schemaStr = this.getSchema({asString: true});
      this._hs = utils.getHash(schemaStr).toString('binary');
    }
    return new Buffer(this._hs, 'binary');
  } else {
    return utils.getHash(this.getSchema({asString: true}), algorithm);
  }
};
...
LogicalType.prototype.toJSON = function () {
  var schema = this.getUnderlyingType().toJSON();
  if (EXPORT_ATTRS) {
...
function Field(schema, opts) {
  ...
  this._aliases = schema.aliases || [];
  this._doc = schema.doc ? '' + schema.doc : undefined;
 ...
 *  + `exportAttrs` {Boolean} Include documentation, field and logical type
 *    attributes.
