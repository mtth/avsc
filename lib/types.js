...
AbstractLongType.prototype.random = function () {
  return this._copy(LongType.prototype.random());
};
...
function UnionType(attrs, opts) {
  ...
  Type.call(this);
  this._types = attrs.map(function (obj) {
...
function MapType(attrs, opts) {
  ...
  opts = getOpts(attrs, opts);
  Type.call(this);
  this._values = Type.fromSchema(attrs.values, opts);
}
...
function ArrayType(attrs, opts) {
  ...
  this._items = Type.fromSchema(attrs.items, opts);
  Type.call(this);
}
...
function Field(attrs, opts) {
  ...
  this._name = name;
  this._type = Type.fromSchema(attrs.type, opts);
...
function resolveNames(attrs, namespace) {
  ...
  var name = attrs.name;
  if (!name) {
    throw new Error(f('missing name in schema: %j', attrs));
  }
  return {
    name: qualify(name),
