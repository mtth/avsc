...
function UnwrappedUnionType(attrs, opts) {
  ...
  this._types.forEach(function (type, index) {
    ...
    if (this._bucketIndices[bucket] !== undefined) {
      throw new Error(f('unambiguous unwrapped union: %j', this));
    }
    ...
    switch (coerce) {
      case 1:
        if (isJsonBuffer(val) && this._bucketIndices.bytes !== undefined) {
          // Note that this can cause corruption if the union also contains a
          // record which matches a buffer's JSON representation (the second
          // predicate is meant to make this case less likely, but this can
          // still happen).
...
module.exports = {
  ...
  builtins: (function () {
    ...
    var i, l, typeName, Class;
    for (i = 0, l = typeNames.length; i < l; i++) {
      typeName = typeNames[i];
      Class = TYPES[typeName];
      if (NAME_PATTERN.test(typeName)) {
        types[Class.name_] = Class;
      }
    }
    return types;
  })()
