...
Type.combine = utils.abstractFunction;

/** Type factory method. */
Type.create = function (schema, opts) {
  if (schema === null) {
  ...
  if (typeof schema == 'string') { // Type reference.
    ...
    if (isPrimitive(schema)) {
      ...
      // reference.
      return opts.registry[schema] = Type.create({type: schema}, opts);
    }
...
Type.inferAttributes = utils.abstractFunction;
...
Type.parseAttributes = utils.abstractFunction;
...
function UnionType(schema, opts) {
  ...
  this._types = schema.map(function (obj) { return Type.create(obj, opts); });
...
function MapType(schema, opts) {
  ...
  this._values = Type.create(schema.values, opts);
}
...
function ArrayType(schema, opts) {
  ...
  this._items = Type.create(schema.items, opts);
}
...
function LogicalType(schema, opts) {
  ...
  Type.call(this);
  LOGICAL_TYPE = this;
  this._underlyingType = Type.create(schema, opts);
}
...
function Field(schema, opts) {
  ...
  this._name = name;
  this._type = Type.create(schema.type, opts);
  this._aliases = schema.aliases || [];
