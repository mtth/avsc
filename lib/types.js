...
Type.isType = function (/* any, [typeName] ... */) {
  ...
  // We check if at least one of the type names matches.
  var typeName = any.getTypeName();
  var i;
  for (i = 1; i < l; i++) {
    if (~typeName.indexOf(arguments[i])) {
      return true;
    }
  }
  return false;
...
Type.prototype.getName = function (asBranch) {
  ...
  if (Type.isType(this, 'logical')) {
    return this._underlyingType.getName(true);
  }
  if (!Type.isType(this, 'union', 'request')) {
...
LogicalType.prototype.getTypeName = function () {
  return '(logical:' + this._underlyingType.getTypeName() + ')';
};
