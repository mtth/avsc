 ...
 * `Number.MAX_SAFE_INTEGER`). See `LongType.using` method above. Note that we
    ...
    return JSON.stringify(obj, function (key, value) {
      if (value) {
        if (
          ...
          // This is a field.
          var order = value.order === 'ascending' ? undefined : value.order;
          var aliases = value.aliases.length ? value.aliases : undefined;
          return {
            name: value.name,
            type: value.type,
            'default': EXPORT_ATTRS ? value['default'] : undefined,
            order: EXPORT_ATTRS ? order : undefined,
            aliases: EXPORT_ATTRS ? aliases : undefined
          };
        } else if (value.name) {
          var name = value.name;
