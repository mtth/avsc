 ...
 *
 */
Type.prototype._skip = /* istanbul ignore next */ function () {
 ...
 *
 */
Type.prototype._generateReader = /* istanbul ignore next */ function (type) {
 ...
 *
 */
Type.prototype.getAdapter = function (type) {

  if (type instanceof UnionType) {
    throw new Error('not yet supported');
  }

  var reader = this._generateReader(type);
  if (!reader) {
    throw new AvscError('incompatible types: %j %j', this, type);
  }
  return new Adapter(this, type, reader);
...
PrimitiveType.prototype._generateReader = function (type) {
...
EnumType.prototype._generateReader = function (type) {
...
FixedType.prototype._generateReader = function (type) {
...
MapType.prototype._generateReader = function (type) {

  type = type || this;
  if (type instanceof MapType) {
    var reader = Tap.prototype.readMap;
    var valuesReader = this.values._generateReader(type.values);
    if (valuesReader) {
...
ArrayType.prototype._generateReader = function (type) {

  type = type || this;
  if (type instanceof ArrayType) {
    var reader = Tap.prototype.readArray;
    var itemsReader = this.items._generateReader(type.items);
    if (itemsReader) {
...
function RecordType(schema, opts) {
  ...
  this._skip = this._generateSkipper();
...
RecordType.prototype._generateReader = function (type) {
  ...
  for (i = 0; i < this.fields.length; i++) {
    ...
    if (!matches.length) {
      ...
      name = matches[0];
      readers[name] = {
        reader: field.type._generateReader(wFields[name]),
        target: field.name
      };
    }
  }
...
RecordType.prototype._generateSkipper = function () {
 ...
 *
 */
function Adapter(readerType, writerType, fn) {
