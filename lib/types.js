...
// TODO: Implement better `clone` support for unwrapped unions.
...
UnwrappedUnionType.prototype.clone = function (obj, opts) {
  if (opts && opts.coerceUnions) {
    // Special case (mostly for defaults), only allow first element.
    return this.types[0].clone(obj, opts);
  }
  var i, l, copy;
  for (i = 0, l = this.types.length; i < l; i++) {
    try {
...
UnwrappedUnionType.prototype._createResolver = function (type, opts) {
  ...
  for (i = 0, l = this.types.length; i < l; i++) {
    ...
    // We don't return `resolver._read` directly because this property might
    // not be available yet (when the schema is recursive).
...
function Field(schema, opts) {
  ...
  if (value !== undefined) {
    var type = this.type;
    var obj = type.clone(value, {coerceBuffers: true, coerceUnions: true});
...
Field.prototype.toJSON = function () {
  return {name: this.name, type: this.type, 'default': this.getDefault()};
};
