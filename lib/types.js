...
Type.fromSchema = function (schema, opts) {
  ...
  type = (function (wrap) {
    ...
    switch (schema.type) {
      ...
      case 'bytes':
      case 'string':
        // We create new primitives here to make type hooks useful (otherwise
        // it wouldn't be possible to make local modification). Primitives
        // referred to by name are still shared though (the most common case).
...
function RecordType(schema, opts) {
  ...
  this.fields = schema.fields.map(function (field) {
    if (typeof field != 'object' || typeof field.name != 'string') {
      throw new AvscError('invalid %s field: %j', this.name, field);
    }
...
RecordType.prototype._createAdapter = function (type, opts) {
  ...
  for (i = 0; i < this.fields.length; i++) {
    ...
    if (!matches.length) {
      ...
      rFieldNames.push('undefined');
    } else {
      name = matches[0];
      adapters[name] = {
        adapter: field.type.createAdapter(wFields[name].type, opts),
        name: field.name // Reader field name.
      };
      rFieldNames.push(field.name);
    }
  }

  // Find if we can add a bypass for unused fields.
  var lazyIndex = -1;
  i = type.fields.length;
  while (i && adapters[type.fields[--i].name] === undefined) {
    lazyIndex = i;
  }
  ...
  body +=  '  return new ' + uname + '(' + rFieldNames.join() + ');\n};';
  return new Function(args.join(), body).apply(undefined, values);
