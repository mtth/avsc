...
// TODO: Forbid use of logical types inside unwrapped unions. And make sure
// that the keys are correct for logical types inside wrapped unions.
...
Type.prototype.getName = function (noUndef) {
  return this._name || (noUndef ? this._typeName : undefined);
};
...
function UnionType(attrs, opts) {
  ...
  this._types.forEach(function (type, i) {
    ...
    var branch = type._name || type._typeName;
  ...
  if (val === null) {
    ...
    switch (coerce) {
      case 3:
        ...
        var obj = {};
        obj[type._name || type._typeName] = type._copy(val, opts);
        return obj;
      default:
        return type._copy(val, opts);
    }
