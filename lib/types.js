...
// TODO: Add `keepLogicalAttributes` option to `getSchema`.
...
// Whether to include logical attributes when getting a type's schema. Using a
// global variable is the simplest way to "pass an argument" to JSON
// stringify's replacer function.
var EXPORT_LOGICAL_ATTRS = false;
...
Type.isType = function (/* any, [pattern] ... */) {
  ...
  // We check if at least one of the patterns matches. As a convenience, we
  // also allow passing in a string (in which case a full match is required).
...
Type.prototype.getName = function (asBranch) {
  ...
  if (NAME_PATTERN.test(type._typeName)) {
...
Type.prototype.inspect = function () {
  ...
  if (isPrimitive(this._typeName)) {
    return f('<%s>', ctor.name_);
  } else {
    var obj = JSON.parse(this.getSchema({noDeref: true}));
    if (typeof obj == 'object') {
      obj.type = undefined; // Would be redundant with constructor name.
    }
    return f('<%s %j>', ctor.name_ || ctor.name || 'LogicalType', obj);
  }
};
...
UnwrappedUnionType.name_ = 'UnwrappedUnionType';
...
WrappedUnionType.name_ = 'WrappedUnionType';
...
function LogicalType(attrs, opts) {
  ...
  Type.call(this, '(logical:' + this._logicalTypeName + ')');
  LOGICAL_TYPE = this;
  this._underlyingType = createType(attrs, opts);
}
util.inherits(LogicalType, Type);
LogicalType.name_ = 'LogicalType';

LogicalType.prototype.getTypeName = function () {
  // We add a suffix for the underlying type to give full flexibility to let
  // `Type.isType` disambiguate between different instances of this type.
  var suffix = this._underlyingType.getTypeName();
  return '(logical:' + this._logicalTypeName + ':' + suffix + ')';
};
...
LogicalType.prototype._export = function (attrs) { return attrs; };
...
Field.prototype.toJSON = function () {
  return {name: this._name, type: this._type, 'default': this.getDefault()};
};
 ...
 *  + `keepDefaults` {Boolean} Include field defaults.
 *  + `noDeref` {Boolean} Always reference types by name when possible,
 *    rather than expand it the first time it is encountered.
 *
 */
function stringify(obj, opts) {
  EXPORT_LOGICAL_ATTRS = opts && opts.exportLogicalAttrs;
  var keepDefaults = opts && opts.keepDefaults;
  ...
  return (function (registry) {
    return JSON.stringify(obj, function (key, value) {
      if (value) {
        if (typeof value == 'object' && value.hasOwnProperty('default')) {
          // This is a field.
          return {
            name: value.name,
            type: value.type,
            'default': keepDefaults ? value['default'] : undefined
          };
        } else if (value.name) {
          var name = value.name;
...
module.exports = {
  ...
  builtins: (function () {
    ...
    for (i = 0, l = typeNames.length; i < l; i++) {
      ...
      types[TYPES[typeName].name_] = TYPES[typeName];
    }
    return types;
  })()
};
