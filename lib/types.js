...
PrimitiveType.prototype._check = function (obj, path, cb) {
...
PrimitiveType.prototype.clone = function (obj) {
  assertValid(this, obj);
  return obj;
};
PrimitiveType.prototype.compare = utils.compare;
PrimitiveType.prototype._isValid = utils.abstractFunction;
...
NullType.prototype._isValid = function (o) { return o === null; };
NullType.prototype._read = function () { return null; };
NullType.prototype._skip = function () {};
NullType.prototype._write = function (tap, o) { assertValid(this, o); };
...
BooleanType.prototype._isValid = function (o) {
...
BooleanType.prototype._write = function (tap, o) {
  assertValid(this, o);
  tap.writeBoolean(o);
};
...
LongType.prototype._isValid = function (o) {
...
UnionType.prototype._check = function (obj, path, cb) {
  var b = false;
  if (obj === null) {
    ...
    if (keys.length === 1) {
      ...
      if (index !== undefined) {
        return this._types[index]._check(obj[name], path.concat(name), cb);
      }
    }
  }
  if (!b && cb) {
...
UnionType.prototype._write = function (tap, obj) {
  ...
  if (obj === null) {
    ...
    if (index === undefined) {
      throw new Error(f('invalid %j value: null', this));
    }
    tap.writeLong(index);
  } else {
    ...
    if (index === undefined) {
      throw new Error(f('invalid %j value: %j', this, obj));
    }
    tap.writeLong(index);
...
UnionType.prototype.clone = function (obj, opts) {
  if (opts && opts.wrapUnions) {
    ...
    assertValid(this, obj); // Will throw.
  }
};
...
EnumType.prototype._isValid = function (s) {
  return this._indices[s] !== undefined;
};

EnumType.prototype._check = PrimitiveType.prototype._check;
...
FixedType.prototype._isValid = function (buf) {
  return Buffer.isBuffer(buf) && buf.length == this._size;
};

FixedType.prototype._check = PrimitiveType.prototype._check;
...
MapType.prototype._check = function (obj, path, cb) {
  ...
  var keys = Object.keys(obj);
  var childPath = path.concat('');
  var b = true;
  var i, l, key;
  for (i = 0, l = keys.length - 1; i <= l; i++) {
    key = keys[i];
    childPath[l] = key; // Reuse array.
    if (!this._values._check(obj[key], childPath, cb)) {
      if (!cb) {
        // We can return early when there is no callback.
        return false;
      }
      b = false;
    }
...
MapType.prototype._write = function (tap, obj) {
  if (!obj || typeof obj != 'object' || obj instanceof Array) {
    throw new Error(f('invalid map: %j', obj));
  }
...
ArrayType.prototype._check = function (obj, path, cb) {
  ...
  for (i = 0, l = obj.length - 1; i <= l; i++) {
    childPath[l] = '' + i; // Reuse array.
    if (!this._items._check(obj[i], childPath, cb)) {
      if (!cb) {
        return false;
      }
      b = false;
...
ArrayType.prototype._write = function (tap, arr) {
  if (!(arr instanceof Array)) {
    throw new Error(f('invalid array: %j', arr));
  }
...
RecordType.prototype._createChecker = function () {
  ...
  var names = [];
  var values = [];
  var body = 'return function check' + unqualify(this._name) + '(obj, path, cb) {\n';
  body += '  if (obj === null || typeof obj != \'object\') {\n';
  body += '    if (cb) { cb(obj, this, path); }\n';
  body += '    return false;\n';
  body += '  }\n';
  body += '  var b = true;\n';
  body += '  var l = path.length;\n';
  body += '  var childPath = path.concat(\'\');\n';
  var i, l, field;
  for (i = 0, l = this._fields.length; i < l; i++) {
    field = this._fields[i];
    names.push('t' + i);
    values.push(field._type);
    body += '  childPath[l] = \'' + field._name + '\';\n';
    if (field.getDefault() === undefined) {
      body += '  if (!t' + i + '._check(obj.' + field._name + ', childPath, cb)) {\n';
    } else {
      body += '  var v' + i + ' = obj.' + field._name + ';\n';
      body += '  if (v' + i + ' !== undefined && ';
      body += '!t' + i + '._check(v' + i + ', childPath, cb)) {\n';
    }
    body += '    if (!cb) { return false; }\n';
    body += '    b = false;\n';
    body += '  }\n';
  }
  body += '  return b;\n};';
