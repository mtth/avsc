...
class Type {
  ...
  createResolver (type, opts) {
    ...
    if (!resolver._read) {
      // TODO: revisit if there are more cases where we need to pretty-print an
      // object as a string
  ...
  toBuffer (val) {
    ...
    this._write(TAP, val);
    if (TAP.isValid()) {
      return Uint8Array.prototype.slice.call(TAP.buf, 0, TAP.pos);
    }
    let buf = utils.newBuffer(TAP.pos);
  ...
  _attrs (opts) {
    ...
    let derefedSchema = this._deref(schema, opts);
...
class UnionType extends Type {
  constructor (schema, opts) {
    // TODO: should super be called with schema and opts?
    super();
  ...
  _deref (schema, opts) {
    return this.types.map((t) => { return t._attrs(opts); });
  }
...
class FixedType extends Type {
  ...
  _check (val, flags, hook) {
    let b = Buffer.isBuffer(val) && val.length === this.size;
    if (!b && hook) {
      hook(val, this);
    }
    return b;
...
class MapType extends Type {
  constructor (schema, opts) {
    // TODO: should schema and opts be passed into super?
    super();
    if (!schema.values) {
  ...
  _createWriter () {
    ...
    for (let i = 0, l = this.fields.length; i < l; i++) {
      ...
      if (field.defaultValue() === undefined) {
        ...
        let value = field.type.toBuffer(field.defaultValue()).toString('binary');
...
class LogicalType extends Type {
  constructor (schema, opts) {
    super();
    // TODO: moved below super(). Should be correct still, but not 100% sure
  ...
  _deref (schema, opts) {
    let type = this.underlyingType;
    let isVisited = type.name !== undefined && opts.derefed[type.name];
