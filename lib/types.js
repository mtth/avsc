...
function EnumType(attrs, opts) {
  ...
  var resolutions = resolveNames(attrs, opts);
...
RecordType.prototype._createReader = function () {
  ...
  var body = 'return function read' + unqualify(this._name) + '(t) {\n';
  body += '  return new R(';
  body += names.map(function (t) { return t + '._read(t)'; }).join();
  body += ');\n};';
  names.unshift('R');
...
RecordType.prototype._createSkipper = function () {
  // jshint -W054
  var args = [];
  var body = 'return function skip' + unqualify(this._name) + '(t) {\n';
  var values = [];
  var i, l;
  for (i = 0, l = this._fields.length; i < l; i++) {
    args.push('t' + i);
...
RecordType.prototype._createWriter = function () {
  ...
  var args = [];
  var body = 'return function write' + unqualify(this._name) + '(t, v) {\n';
  var values = [];
  var i, l, field, value;
  for (i = 0, l = this._fields.length; i < l; i++) {
    field = this._fields[i];
 ...
 *
 */
function resolveNames(attrs, opts) {
  var namespace = attrs.namespace || opts.namespace;
  ...
  } else if (!opts.allowAnonymousTypes) {
