...
function AvscError(message) {
  ...
  if (l > 1) {
    ...
    this.message = util.format.apply(args);
  } else {
...
function PrimitiveType(name) {
  ...
  switch (name) {
    ...
    case 'boolean': // jshint -W018
      this.isValid = function (o) { return o !== !o; };
...
function WrappedUnionType(schema, opts) {
  ...
  this.isValid = function (obj) {
    ...
    return self.types[index].isValid.call(this, obj[name]);
  };
...
function UnwrappedUnionType(schema, opts) {
  ...
  this.isValid = function (obj) {
    return self.types.some(function (type) { return type.isValid(obj); });
  };
...
function EnumType(schema, opts) {
  ...
  var self = this;
  var names = resolveName(schema, opts.namespace); // Performs name checks.
...
function MapType(schema, opts) {
  ...
  this._read = function () {
    return reader.call(this, self.values._read);
  };

  this._write = function (obj) {
    writer.call(this, obj, self.values._write);
  };
  ...
  this.isValid = function (obj) {
    ...
    for (i = 0, l = keys.length; i < l; i++) {
      if (!self.values.isValid(obj[keys[i]])) {
        return false;
      }
    }
    return true;
...
MapType.prototype.random = function () {
  ...
  for (i = 0, l = Math.floor(randomDouble(0, 5)); i < l; i++) {
    obj[randomString(8)] = this.values.random();
  }
  return obj;
...
function ArrayType(schema, opts) {
  ...
  this._read = function () {
    return reader.call(this, self.items._read);
  };

  this._write = function (arr) {
    writer.call(this, arr, self.items._write);
  };
  ...
  this.isValid = function (obj) {
    ...
    return obj.every(function (elem) { return self.items.isValid(elem); });
  };
...
function RecordType(schema, opts) {
  ...
  this.fields = schema.fields.map(function (field) {
    ...
    if (value !== undefined) {
      if (~['fixed', 'bytes'].indexOf(type.type) && typeof value == 'string') {
    ...
    return {
      ...
      _getDefault: function () { return value; }
    };
  });
  ...
  function generateConstructor() {
    ...
    var body = 'return function ' + names.unqualified + '(';
    body += args.join(',');
    body += ') {\n';
  ...
  function generateReader() {
    ...
    var body = 'return function read' + names.unqualified + '() {\n';
  ...
  function generateWriter() {
    ...
    var names = [];
    var values = [];
    var body = 'return function write' + names.unqualified + '(obj) {\n';
    var i, l, field;
    for (i = 0, l = self.fields.length; i < l; i++) {
      field = self.fields[i];
      names.push('w' + i);
  ...
  function generateChecker() {
    ...
    var names = [];
    var values = [];
    var body = 'return function check' + names.unqualified + '(obj) {\n';
 ...
 *
 */
function resolveName(schema, namespace) {

  var name = schema.name;
  if (!name) {
    throw new AvscError('missing name: %j', schema);
  }
  ...
  if (~PRIMITIVES.indexOf(suffix)) {
    throw new AvscError('cannot rename: %s', suffix);
  }

  return {qualified: name, unqualified: suffix};
