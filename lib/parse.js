...
function RecordType(schema, opts) {
  ...
  if (hasDuplicates(schema.fields.map(function (f) { return f.name; }))) {
  ...
  this.fields = schema.fields.map(function (field) {
    ...
    if (value !== undefined) {
      if (~['fixed', 'bytes'].indexOf(type.type)) {
        if (Buffer.isBuffer(value)) {
          ...
          // back on the field to ensure `toJSON` will work appropriately.
    ...
    // Having the second default in an option has the added benefit that it
    // won't be included in the JSON serialization.
  });
  ...
  // Default reader (used when the writer schema is the same).
