...
function parse(schema, opts) {
  ...
  if (schema instanceof Array) { // Union.
    Type = opts.unwrapUnions ? UnwrappedUnionType : UnionType;
...
function UnionType(schema, opts) {
  ...
  if (!schema.length) {
    throw new ParseError('empty union');
  }
