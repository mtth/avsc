...
/**
 * Default module path resolution hook. Resolves the path of a module relative
 * to another.
 */
function createRelativeResolveHook() {
  return function (sourcePath, importPath) {
    return path.join(path.dirname(sourcePath), importPath);
  };
}

function treatStringAsPath(str) {
  return ~str.indexOf(path.sep) &&
    fs.existsSync(str);
}
...
module.exports = {
  ...
  treatStringAsPath: treatStringAsPath,
