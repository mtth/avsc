  ...
  svc.createClient({timeout: opts && opts.timeout})
    .createChannel(transport, {
      ...
      .once('error', function (err) {
        // Stateless transports will throw an interrupted error when the channel
...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  return channel._emit(wreq, opts, function (err, wres) {
...
Server.prototype.createChannel = function (transport, opts) {
  ...
  channel.once('eot', function () { delete channels[channelId]; });
  this.emit('channel', channel);
  return channel;
};
 ...
 * client channels to store pending calls. This class isn't exposed by the public
 * API.
 */
