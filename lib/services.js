...
Message.forSchema = function (name, schema, opts) {
  ...
  let recordName = f('%s.%sRequest', OPTS.namespace, utils.capitalize(name));
  ...
  if (typeof transport == 'function') {
    ...
    if (objectMode) {
      ...
      writableFactory = function (cb) {
        ...
        let writable = transport(function (err, readable) {
          if (err) {
            cb(err);
            return;
          }
...
Client.prototype._createMessageHandler$ = function (msg) {
  ...
  let names = fields.map(function (f) { return f.getName(); });
...
StatelessClientChannel.prototype._send = function (id, reqBuf) {
  ...
  function emit(retry) {
    ...
    let writable = self._writableFactory.call(self, function (err, readable) {
      if (err) {
        cb(err);
        return;
      }
  ...
  function onTransition(wreq, wres, prev) {
    ...
    if (!handler) {
      ...
      if (defaultHandler) {
        ...
        let cause = new Error(f('no handler for %s', msg.name));
...
function serializationError(msg, obj, fields) {
  ...
  let detailsStr = details
    ...
    .join(', ');
  let err = new Error(f('%s (%s)', msg, detailsStr));
  err.details = details;
  return err;
