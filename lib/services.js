...
// TODO: Remote `getDocumentation` method from types and here?
...
Service.prototype.equals = function (any) {
  return (
    Service.isService(any) &&
    this.getHash().equals(any.getHash())
  );
};
...
function Client(svc, opts) {
  ...
  if (opts.remoteSchemas) {
    Adapter._populateCache(this._cache$, svc, opts.remoteSchemas, true);
  }
...
function Server(svc, opts) {
  ...
  if (opts.remoteSchemas) {
    Adapter._populateCache(this._cache, svc, opts.remoteSchemas, false);
  }
...
function Adapter(clientSvc, serverSvc, fingerprint, isRemote) {
  ...
  this._rsvs = clientSvc.equals(serverSvc) ? null : this._createResolvers();
}
...
Adapter.prototype._getReader = function (name, qualifier) {
  if (this._rsvs) {
    ...
    var msg = this._serverSvc.getMessage(name);
    switch (qualifier) {
