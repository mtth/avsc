 ...
 * onTransition, and onCompletion.
 * @param args {Array} Array of arguments. These will be used for all forward
 * middleware functions.
 * @param onTransition {Function} End of forward phase, its callback accepts an
 * eventual error as single argument. This will be used for the backward phase.
 * This function is guaranteed to be called at most once.
 * @param onCompletion {Function} Final handler, it takes it an error as unique
...
function chainFunctions(fns, ctx, args, onTransition, onCompletion, onError) {
  ...
  function forward(pos) {
    ...
    if (pos < fns.length) {
      ...
      onTransition.apply(ctx, args.concat(function (err) {
        if (isDone) {
  ...
  function backward() {
    ...
    if (cb) {
      var isDone = false;
      cb.call(ctx, cause, function (err) {
        if (isDone) {
