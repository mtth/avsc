...
Client.prototype.createChannel = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    if (objectMode) {
      ...
      writableFactory = function (cb) {
        ...
        var writable = transport(function (err, readable) {
          ...
          // attached, we forward any errors to the client).
          cb(null, readable.pipe(forwardErrors(new FrameDecoder(), this)));
        });
        if (writable) {
    ...
    if (!objectMode) {
      ...
      var decoder = forwardErrors(new NettyDecoder(), this);
  ...
  if (typeof transport == 'function') {
    ...
    if (objectMode) {
      ...
      readableFactory = function (cb) {
        var readable = transport(function (err, writable) {
          if (err) {
            cb(err);
            return;
          }
        ...
        if (readable) {
          return readable.pipe(forwardErrors(new FrameDecoder(), this));
        }
      };
    }
    ...
    if (!objectMode) {
      var decoder = new forwardErrors(new NettyDecoder(), this);
      readable = readable.pipe(decoder);
      var encoder = new NettyEncoder();
      encoder.pipe(writable);
      writable = encoder;
  ...
  process.nextTick(function () {
    ...
    readable = readableFactory.call(this, function (err, writable) {
      if (err) {
        ...
        // We also delay until the next tick in case this call is synchronous.
    ...
    if (readable) {
      readable.on('data', onRequest).on('end', onEnd);
    } else {
      self.emit('error', new Error('invalid readable stream'));
      onFinish();
    }
  });
