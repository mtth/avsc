...
Service.prototype.createClient = function (opts) {
  ...
  process.nextTick(function () {
    ...
    // before any emitters get created.
    if (opts && opts.server) {
...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  return emitter._emit(wreq, opts, function (err, wres) {
    ...
    var errType = wreq._msg.getErrorType();
    if (err) {
...
MessageEmitter.prototype._emit = function (wreq, opts, cb) {
  var msg = wreq._msg;
  var wres = msg.isOneWay() ? undefined : new WrappedResponse({});
  var ctx = this._client._legacy$ ? this : CallContext.forEmitter(msg, this);
  var self = this;
  this._pending++;
  process.nextTick(function () {
    if (!msg.getName()) {
...
MessageListener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  var ctx = this._server._legacy ? this : CallContext.forListener(msg, this);
...
WrappedResponse.prototype.toBuffer = function (msg) {
  ...
  return Buffer.concat([
    ...
    hasError ?
      msg.getErrorType().toBuffer(this._err) :
      msg.getResponseType().toBuffer(this._res)
  ]);
};
 ...
 */
function CallContext(msg, emitter, listener) {
  this._msg = msg;
...
CallContext.forEmitter = function (msg, emitter) {
  return new CallContext(msg, emitter, undefined);
};

CallContext.forListener = function (msg, listener) {
  return new CallContext(msg, undefined, listener);
};
