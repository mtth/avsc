...
Service.prototype.emit = util.deprecate(
  function (name, req, stub, cb) {
    if (!stub || !this.equals(stub.client._svc$)) {
...
Service.prototype.on = util.deprecate(
  function (name, handler) {
    ...
    this._server.onMessage(name, function (req, cb) {
      return handler.call(self, req, this.stub, cb);
    });
    return this;
  },
...
Client.prototype.activeStubs = function () {
...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  // Check that we can send the message now, if not buffer the call.
  var stubs = this._stubs$;
  var numStubs = stubs.length;
  if (!numStubs) {
    if (this._buffering$) {
      debug('no active client stubs, buffering call');
      this.once('stub', function () {
        this._emitMessage(wreq, opts, cb);
      });
    } else {
      var err = new Error('no active stubs');
      if (cb) {
...
Server.prototype.activeStubs = function () {
...
ClientStub.prototype.destroy = function (noWait) {
  ...
  this.destroyed = true;
  // We signal to the client that this stub isn't writable anymore.
  this.emit('_finish');
  ...
  if (noWait || !pending) {
    ...
    debug('client stub entering lame duck mode');
  }
};
...
ClientStub.prototype._emit = function (wreq, opts, cb) {
  ...
  function onCompletion(err) {
    self.pending--;
    if (self.destroyed && !self._interrupted && !self.pending) {
      self.destroy();
    }
...
utils.addDeprecatedGetters(ClientStub, ['pending', 'timeout']);
...
ClientStub.prototype.getProtocol = util.deprecate(
  ...
  'use `client.service` instead'
);

ClientStub.prototype.isDestroyed = util.deprecate(
  function () { return this.destroyed; },
  'use `destroyed` instead'
);
...
ServerStub.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  function onCompletion(err) {
    ...
    if (!resBuf) {
      resBuf = self._encodeSystemError(err, wres && wres.headers);
    } else if (resErr) {
    ...
    if (self.destroyed && !self.pending) {
      self.destroy();
    }
  }
...
utils.addDeprecatedGetters(ServerStub, ['pending']);
...
ServerStub.prototype.getProtocol = util.deprecate(
  ...
  'use `server.service` instead'
);

ServerStub.prototype.isDestroyed = util.deprecate(
  function () { return this.destroyed; },
  'use `destroyed` instead'
);
