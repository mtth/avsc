...
// TODO: Destroy emitter on protocol discovery error.
// TODO: Move from factory method to constructor for building services.
...
Message.prototype.getSchema = Type.prototype.getSchema;
 ...
 * `Service.forProtocol` method. This function performs little logic to
 * better support efficient copy.
 */
function Service(name, messages, types, doc, str, server) {
  ...
  // We keep a reference to the schema to be able to send it over the wire with
  // all non-standard attributes included.
  // TODO: Rename this to _ptclStr;
  this._str = str;
...
Service.forProtocol = function (schema, opts) {
  opts = opts || {};
  ...
  if (schema.messages) {
    ...
    Object.keys(schema.messages).forEach(function (key) {
      msgs[key] = Message.forSchema(key, schema.messages[key], opts);
    });
  }

  var str = JSON.stringify(schema);
  return new Service(name, msgs, opts.registry, schema.doc, str);
};
...
Service.prototype.equals = function (itfc) {
  return (
    Service.isService(itfc) &&
    this.getFingerprint().equals(itfc.getFingerprint())
  );
};

Service.prototype.getDocumentation = function () { return this._doc; };

Service.prototype.getFingerprint = Type.prototype.getFingerprint;
...
Service.prototype.getSchema = Type.prototype.getSchema;
...
Service.prototype.toJSON = Type.prototype.toJSON;
...
Service.prototype._attrs = function (opts) {
  ...
  if (msgNames.length) {
    ...
    msgNames.forEach(function (name) {
      schema.messages[name] = this._messagesByName[name]._attrs(opts);
    }, this);
  }
...
Service.prototype.subprotocol = util.deprecate(
  function () {
    ...
    var server = new Server(parent._itfc, opts);
...
function discoverProtocol(transport, opts, cb) {
  ...
  var itfc = Service.forProtocol({protocol: 'Empty'}, OPTS);
  var client = itfc.createClient();
...
function Client(itfc, opts) {
  ...
  if (opts.remoteSchemas) {
    Adapter._populateCache(this._cache$, itfc, opts.remoteSchemas, true);
  }

  this._itfc$.getMessages().forEach(function (msg) {
...
Client.prototype.emitMessage = function (name, req, opts, cb) {
  ...
  var msg = getExistingMessage(this._itfc$, name);
...
Client.prototype.getService = function () { return this._itfc$; };

Client.prototype.getRemoteSchemas = function () {
  return Adapter._getRemoteSchemas(this._itfc$, this._cache$, true);
};
...
function Server(itfc, opts) {
  ...
  if (opts.remoteSchemas) {
    Adapter._populateCache(this._cache, itfc, opts.remoteSchemas, false);
  }

  itfc.getMessages().forEach(function (msg) {
...
Server.prototype.getService = function () { return this._itfc; };

Server.prototype.getRemoteSchemas = function () {
  return Adapter._getRemoteSchemas(this._itfc, this._cache, false);
};
...
function MessageEmitter(client, opts) {
  ...
  this._timeout = utils.getOption(opts, 'timeout', 10000); // Deprecated.
  this._prefix = normalizedPrefix(opts.scope);
  this._context = opts.context || {};

  var cache = client._cache$;
  var clientItfc = client._itfc$;
  var fgpt = opts.serverFingerprint;
  if (!fgpt) {
    fgpt = clientItfc.getFingerprint();
  }
  var adapter = cache[fgpt];
  if (!adapter) {
    ...
    fgpt = clientItfc.getFingerprint();
    adapter = cache[fgpt] = new Adapter(clientItfc, clientItfc, fgpt);
  }
...
MessageEmitter.prototype._createHandshakeRequest = function (adapter, noItfc) {
  var itfc = this._client._itfc$;
  return {
    clientHash: itfc.getFingerprint(),
    clientProtocol: noItfc ? null : itfc._str,
...
MessageEmitter.prototype._getAdapter = function (hres) {
  var fgpt = hres.serverHash;
  var cache = this._client._cache$;
  var adapter = cache[fgpt];
  if (adapter) {
    return adapter;
  }
  var schema = JSON.parse(hres.serverProtocol);
  ...
  var serverItfc = Service.forProtocol(schema, {wrapUnions: true});
  adapter = new Adapter(this._client._itfc$, serverItfc, fgpt, true);
  return cache[fgpt] = adapter;
};
...
MessageEmitter.prototype.emitMessage = util.deprecate(
  function (name, reqEnv, opts, cb) {
    ...
    var itfc = this._client.getService();
    var msg = itfc.getMessage(name);
    if (!msg) {
      // Deprecated.
      process.nextTick(function () {
        cb.call(itfc, new Error(f('unknown message: %s', name)));
      });
      return;
    }
...
MessageEmitter.prototype.getCache = util.deprecate(
  ...
  'use `getClient().getRemoteSchemas()` instead'
);
...
function MessageListener(server, opts) {
  ...
  var itfc = server.getService();
  var fgpt = itfc.getFingerprint();
  if (!cache[fgpt]) {
    ...
    cache[fgpt] = new Adapter(itfc, itfc, fgpt);
  }
...
MessageListener.prototype._getAdapter = function (hreq) {
  ...
  var schema = JSON.parse(hreq.clientProtocol);
  // See `MessageEmitter._getAdapter` for `wrapUnions` motivation.
  var clientItfc = Service.forProtocol(schema, {wrapUnions: true});
  adapter = new Adapter(clientItfc, this._server.getService(), fgpt, true);
  return this._server._cache[fgpt] = adapter;
};
...
MessageListener.prototype.getCache = util.deprecate(
  ...
  'use `getServer().getRemoteSchemas()` instead'
);
 ...
 */
function Adapter(clientItfc, serverItfc, fingerprint, isRemote) {
  this._clientItfc = clientItfc;
  this._serverItfc = serverItfc;
  this._fingerprint = fingerprint; // Convenience.
  this._isRemote = !!isRemote;
  this._rsvs = clientItfc.equals(serverItfc) ? null : this._createResolvers();
}

Adapter._populateCache = function (cache, itfc, schemas, isClient) {
  Object.keys(schemas).forEach(function (fgpt) {
    var schema = schemas[fgpt];
    var clientItfc, serverItfc;
    if (isClient) {
      clientItfc = itfc;
      serverItfc = Service.forProtocol(schema, {wrapUnions: true});
    } else {
      clientItfc = Service.forProtocol(schema, {wrapUnions: true});
      serverItfc = itfc;
    }
    cache[fgpt] = new Adapter(clientItfc, serverItfc, fgpt, true);
  });
  return cache;
};

Adapter._getRemoteSchemas = function (itfc, cache, isClient) {
  var schemas = {};
  Object.keys(cache).forEach(function (hs) {
    var adapter = cache[hs];
    if (adapter._isRemote) {
      var itfc = isClient ? adapter._serverItfc : adapter._clientItfc;
      schemas[hs] = JSON.parse(itfc._str);
    }
  });
...
Adapter.prototype._createResolvers = function () {
  var rsvs = {};
  this._clientItfc.getMessages().forEach(function (c) {
    var n = c.getName();
    var s = this._serverItfc.getMessage(n);
    if (!s) {
...
Adapter.prototype._getReader = function (name, qualifier) {
  if (this._rsvs) {
    ...
    var msg = this._serverItfc.getMessage(name);
    switch (qualifier) {
...
Adapter.prototype._decodeRequest = function (buf) {
  ...
  var msg, req;
  if (name) {
    msg = this._serverItfc.getMessage(name);
...
Adapter.prototype._decodeResponse = function (buf, wres) {
  ...
  if (name) {
    ...
    msg = this._clientItfc.getMessage(name);
    if (isError) {
...
Adapter.prototype.getClientProtocol = util.deprecate(
  function () { return this._clientItfc; },
  'use `server.getRemoteSchemas()` instead.'
);

Adapter.prototype.getServerProtocol = util.deprecate(
  function () { return this._serverItfc; },
  'use `client.getRemoteSchemas()` instead.'
);
 ...
 *
 * @param itfc {Service} The protocol to look into.
 * @param name {String} The message's name.
 */
function getExistingMessage(itfc, name) {
  var msg = itfc.getMessage(name);
  if (!msg) {
