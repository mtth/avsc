...
function Client(svc, opts) {
  ...
  // using the emitter itself as context rather than a per-call context).
  this._legacy$ = false;
...
Server.prototype.onMessage = function (name, handler) {
  getExistingMessage(this.getService(), name); // Check message existence.
...
MessageEmitter.prototype.ping = function (timeout, cb) {
  ...
  this._emit(wreq, {timeout: timeout}, cb || function (err) {
    if (err) {
      self.emit('error', err);
    }
  });
...
MessageEmitter.prototype._emit = function (wreq, opts, cb) {
  ...
  var ctx = this._client._legacy$ ?
    this :
    CallContext.forEmitter(this, this._ctxOpts, opts);
  var self = this;
  this._pending++;
  process.nextTick(function () {
    if (!wreq.getMessage().getName()) {
      ...
      debug('starting emitter middleware chain (%s middleware)', fns.length);
      chainMiddleware({
        fns: fns,
        ctx: ctx,
        wreq: wreq,
    ...
    if (self._destroyed) {
      ...
      try {
        ...
        err = serializationError(
          f('invalid %s request', msg.getName()),
          'req',
          wreq.getRequest(),
          msg.getRequestType()
        );
...
function StatelessEmitter(client, writableFactory, opts) {
  ...
  if (!opts || !opts.noPing) {
    ...
    // If not, this will throw an error on the emitter.
...
MessageListener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  var ctx = this._server._legacy ?
    this :
    CallContext.forListener(this, this._ctxOpts);
  var fns = this._server._fns;
  debug('starting listener middleware chain (%s middleware)', fns.length);
  self._pending++;
  chainMiddleware({
    fns: fns,
    ctx: ctx,
  ...
  function onCompletion(err) {
    ...
    if (!err) {
      ...
      if (!isStrict) {
        if (isError(resErr)) {
          wres.setError(
            ...
            msg.getErrorType().clone(resErr.message, {wrapUnions: true})
          );
        } else if (resErr === null) {
      ...
      try {
        ...
        if (wres.hasError()) {
          err = serializationError(
            f('invalid %s error', msg.getName()), // Sic.
            'err',
            wres.getError(),
            msg.getErrorType()
          );
          ...
          err = serializationError(
            f('invalid %s response', msg.getName()),
            'res',
            wres.getResponse(),
            msg.getResponseType()
          );
 ...
 */
MessageListener.prototype._encodeSystemError = function (err, header) {
 ...
 */
function CallContext(emitter, listener, ctxOpts, callOpts) {
  this._emitter = emitter;
  this._listener = listener;
  this._ctxOpts = ctxOpts;
  this._callOpts = callOpts;
  this._locals = {};
}

CallContext.forEmitter = function (emitter, contextOpts, callOpts) {
  return new CallContext(emitter, undefined, contextOpts, callOpts);
};

CallContext.forListener = function (listener, contextOpts) {
  return new CallContext(undefined, listener, contextOpts);
};

CallContext.prototype.getCallOptions = function () { return this._callOpts; };

CallContext.prototype.getContextOptions = function () {
  return this._ctxOpts;
};

CallContext.prototype.getEmitter = function () { return this._emitter; };

CallContext.prototype.getListener = function () { return this._listener; };
