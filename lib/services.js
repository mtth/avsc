...
// TODO: Rename `strictErrors` to `strictTypes` and allow the default to coerce
// `undefined` responses into `null`.
...
function Client(svc, opts) {
  ...
  Object.seal(this); // Can't freeze event emitters.
}
...
ServerChannel.prototype._encodeSystemError = function (err, header) {
  ...
  if (server._sysErrFormatter) {
    ...
    // Error messages are traditionally lower-cased rather than enum-like, we
    // massage the RPC code to have a more idiomatic feel.
    errStr = err.rpcCode.toLowerCase().replace(/_/, ' ');
  }
  var hdrBuf;
  if (header) {
    try {
  ...
  return Buffer.concat([
    ...
    STRING_TYPE.toBuffer(errStr || 'remote error')
  ]);
};
...
function serializationError(msg, name, val, type) {
  ...
  var detailsStr = details.map(function (obj) {
    return f('%s%s = %j but expected %s', name, obj.path, obj.value, obj.type);
  }).join(', ');
  ...
  function errorHook(parts, any, type) {
    ...
    details.push({path: strs.join(''), value: any, type: type});
  }
}
  ...
  function forward(pos) {
    ...
    if (pos < params.fns.length) {
      ...
      params.onTransition.apply(params.ctx, args.concat(function (err) {
        if (isDone) {
          params.onError(new Error('duplicate middleware final call'));
          return;
        }
        isDone = true;
        cause = err;
