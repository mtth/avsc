...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  if (!numChannels) {
    if (this._buffering$) {
      ...
      var err = toRpcError('NO_ACTIVE_CHANNELS');
      process.nextTick(function () {
        if (cb) {
...
ClientChannel.prototype._emit = function (wreq, opts, cb) {
  ...
  function onTransition(wreq, wres, prev) {
    ...
    var err, reqBuf;
    if (self.destroyed) {
      err = toRpcError('CHANNEL_DESTROYED');
    } else {
      try {
...
function StatefulClientChannel(client, readable, writable, opts) {
  ...
  if (this._connected) {
    ...
    if (self.timeout) {
      timer = setTimeout(function () {
        self.destroy(toRpcError('TIMEOUT'));
      }, self.timeout);
    }
  }
  ...
  function onPing(obj) {
    ...
    try {
      ...
      self.destroy(toRpcError('INVALID_HANDSHAKE_RESPONSE', cause));
      return;
    }
    debug('handshake match: %s', hres.match);
    self.emit('handshake', hreq, hres);
...
Registry.prototype.add = function (timeout, fn) {
  ...
  var timer;
  if (timeout > 0) {
    timer = setTimeout(function () { cb(toRpcError('TIMEOUT')); }, timeout);
  }
