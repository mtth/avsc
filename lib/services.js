...
Client.prototype._emitMessage = function (wreq, opts, cb) {
  ...
  return emitter._emit(wreq, opts, function (err, wres) {
    ...
    if (!self._strict$) {
      ...
      err = err === undefined ? null : (asSystemError(err, errType) || err);
    }
...
Server.prototype._onError = function (err) {
  ...
  if (!this._silent && err.rpcCode !== 'UNKNOWN_PROTOCOL') {
    console.error();
    console.error(err.rpcCode || 'SYSTEM_ERROR');
...
MessageEmitter.prototype._emit = function (wreq, opts, cb) {
  ...
  var ctx = expandOption(this._client._ctx$, [this, this._ctxOpts, opts]);
  var self = this;
  this._pending++;
  process.nextTick(function () {
...
MessageListener.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  try {
    ...
    cb(self._encodeError(toRpcError('INVALID_REQUEST', cause)));
    return;
  }
  ...
  var ctx = expandOption(this._server._ctx, [this, this._ctxOpts]);
  ...
  function onCompletion(err) {
    ...
    if (!err) {
      ...
      var isStrict = self._server._strict;
      if (!isStrict && isError(resErr)) {
        err = resErr;
      } else if (!(err = asSystemError(resErr, msg.getErrorType()))) {
        if (!isStrict && resErr === null) {
...
MessageListener.prototype._encodeError = function (err, header) {
  ...
  return Buffer.concat([
    ...
    STRING_TYPE.toBuffer(errStr || err.rpcCode || err.message)
  ]);
};
...
/**
 * Tries to coerce an Avro response "system error" into a JavaScript error.
 *
 * @param err {...} The error in the response to extract a system error from. A
 * system error corresponds to the first type in the union (string). If the
 * error in the response is of another type, no extraction will happen and
 * `undefined` will be returned.
 * @param type {Type} Error type for this response.
 */
function asSystemError(err, type) {
  var errStr;
  if (Type.isType(type, 'union:unwrapped')) {
  ...
  if (errStr !== undefined) {
    var sysErr = new Error(errStr.split(/\n/, 1)[0]);
    sysErr.originalMessage = errStr;
    return sysErr;
  }
}
