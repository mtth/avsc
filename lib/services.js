...
class Service {
  ...
  get hash () { return Buffer.from(this._hashStr, 'binary'); }
  ...
  constructor (client, readable, writable, opts) {
    ...
    function ping(retry) {
      ...
      let payload = [
        ...
        Buffer.from([0, 0]) // No header, no data (empty message name).
      ];
...
class ServerChannel extends events.EventEmitter {
  ...
  _createHandshakeResponse (err, hreq) {
    ...
    let buf = svc.hash;
    let serverMatch = hreq && Buffer.compare(hreq.serverHash, buf) === 0;
    return {
  ...
  _encodeSystemError (err, header) {
    ...
    return Buffer.concat([
      hdrBuf || Buffer.from([0]),
      Buffer.from([1, 0]), // Error flag and first union index.
...
class FrameDecoder extends stream.Transform {
  ...
  _transform (buf, encoding, cb) {
    ...
    while (
      ...
      if (frameLength) {
        this._bufs.push(buf.subarray(4, frameLength + 4));
      } else {
        let bufs = this._bufs;
        this._bufs = [];
        this.push({id: null, payload: bufs});
      }
      buf = buf.subarray(frameLength + 4);
    }
    this._buf = buf;
    cb();
  }
...
function readHead(type, buf) {
  ...
  return {head, tail: Buffer.from(tap.subarray(tap.pos))};
}
