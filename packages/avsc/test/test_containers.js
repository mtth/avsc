...
suite('containers', function () {
  ...
  suite('encode & decode', function () {
    ...
    test('empty block', function (cb) {
      ...
      decoder.write(HEADER_TYPE.binaryEncode({
        magic: MAGIC_BYTES,
        meta: {
          'avro.schema': utils.bufferFrom('"int"'),
          'avro.codec': utils.bufferFrom('null')
      ...
      decoder.write(BLOCK_TYPE.binaryEncode({
        count: 1, data: t.binaryEncode(1), sync: SYNC
      }));
      decoder.write(BLOCK_TYPE.binaryEncode({
        count: 0, data: utils.bufferFrom([]), sync: SYNC
      }));
      decoder.write(BLOCK_TYPE.binaryEncode({
        count: 1, data: t.binaryEncode(2), sync: SYNC
      }));
      decoder.end();
    });
