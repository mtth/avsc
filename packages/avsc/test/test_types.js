...
suite('types', function () {
  ...
  suite('BytesType', function () {
    ...
    test('clone', function () {
      ...
      assert.throws(function () { t.clone(1); });
    });
  ...
  suite('UnwrappedUnionType', function () {
    ...
    test('fromJSON', function () {
      var type = new builtins.UnwrappedUnionType(['null', 'int']);
      assert.strictEqual(type.jsonDecode(null), null);
      assert.deepEqual(type.jsonDecode({int: 48}), 48);
      assert.throws(function () { type.jsonDecode(48); });
      assert.throws(function () { type.jsonDecode({long: 48}); });
    });

    test('toJSON', function () {
      var type = new builtins.UnwrappedUnionType(['null', 'int']);
      assert.strictEqual(type.jsonEncode(null), null);
      assert.deepEqual(type.jsonEncode(48), {int: 48});
      assert.throws(function () { type.jsonEncode(2.5); });
    });
    ...
    test('schema', function () {
      var type = new builtins.UnwrappedUnionType(['null', 'int']);
      assert.equal(JSON.stringify(type.schema()), '["null","int"]');
    });
    ...
    test('invalid null', function () {
      ...
      assert.throws(function () { t.jsonDecode(null); });
    });

    test('invalid multiple keys', function () {
      var t = new builtins.UnwrappedUnionType(['null', 'int']);
      var o = {'int': 2};
      assert.equal(t.jsonDecode(o), 2);
      o.foo = 3;
      assert.throws(function () { t.jsonDecode(o); });
    });
  ...
  suite('RecordType', function () {
    ...
    test('fromJSON', function () {
      ...
      assert.throws(function () { t.jsonDecode({}); });
    });

    test('toJSON record', function () {
      var T = Type.forSchema({
        type: 'record',
        name: 'Person',
      ...
      assert.deepEqual(r.toJSON(), T.type.jsonEncode(r));
    });
  ...
  suite('AbstractLongType', function () {
    ...
    suite('unpacked', function () {
      ...
      test('clone', function () {
        ...
        assert.equal(slowLongType.clone(123, {}), 123);
        assert.equal(slowLongType.jsonDecode(-1), -1);
        assert.equal(slowLongType.jsonEncode(-1), -1);
      });
  ...
  suite('LogicalType', function () {
    ...
    function DateType(schema, opts) {
      ...
      if (!types.Type.isType(this.underlyingType, 'long', 'string')) {
    ...
    test('even integer', function () {
      ...
      assert.throws(function () { t.jsonDecode(5); });
  ...
  suite('toJSON', function () {
    ...
    test('int', function () {
      ...
      assert.throws(function () { t.jsonEncode('a'); });
    });
  ...
  data.forEach(function (elem) {
    test('roundtrip', function () {
      ...
      elem.valid.forEach(function (v) {
        ...
        fn(type.jsonDecode(type.jsonEncode(v)), v);
      });
