...
class Type {
            ...
            case 'object': {
                if (val === null) {
                    ...
                    return Type.forSchema({
                        type: 'array',
                        items: Type.forTypes(val.map((v) => { return Type.forValue(v, opts); }), opts)
                    }, opts);
                }
    ...
    createResolver(type, opts) {
        ...
        opts.registry = opts.registry || {};
        let resolver, key;
        if (Type.isType(this, 'record', 'error') &&
    ...
    equals(type, opts) {
        ...
        return (JSON.stringify(this.schema({ exportAttrs: true })) ===
...
class RecordType extends Type {
    ...
    _createConstructor(errorStack, plainRecords) {
        ...
        for (let i = 0, l = this.fields.length; i < l; i++) {
            ...
            let name = field.name;
            if (errorStack && this._isError && name === 'stack' &&
    ...
    _update(resolver, type, opts) {
        ...
        for (let i = 0; i < rFields.length; i++) {
            ...
            if (matches.length > 1) {
                throw new Error(`ambiguous aliasing for ${type.name}.${field.name} (${matches.join(', ')})`);
            }
            if (!matches.length) {
                if (field.defaultValue() === undefined) {
                    throw new Error(`no matching field for default-less ${type.name}.${field.name}`);
                }
