...
WrappedUnionType.prototype.compare = function (val1, val2, opts) {
  ...
  if (name1 === name2) {
    return name1 === 'null' ?
      0 :
      this.types[index].compare(val1[name1], val2[name1], opts);
  } else {
...
MapType.prototype.compare = function (val1, val2, opts) {
  ...
  for (i = 0, l = Math.min(n1, n2); i < l; i++) {
    k = keys1[i];
    if ((f = utils.compare(k, keys2[i]))) {
      return f;
    }
    if ((f = this.valuesType.compare(val1[k], val2[k], opts))) {
      return f;
    }
  }
...
ArrayType.prototype.compare = function (val1, val2, opts) {
  ...
  var i, l, f;
  for (i = 0, l = Math.min(n1, n2); i < l; i++) {
    if ((f = this.itemsType.compare(val1[i], val2[i], opts))) {
      return f;
    }
  }
  return utils.compare(n1, n2);
 ...
 * `recordConstructor` property. This "specific record class" gives significant
