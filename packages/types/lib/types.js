...
// Encoding writer (shared for performance).
var WRITER = new bytes.Writer(new buffer.SlowBuffer(1024));

// Coercions.
COERCE_JSON_BUFFERS = 1;
...
Type.prototype.copy = function (val) {
...
Type.prototype.compareBuffers = function (buf1, buf2) {
  return this._match(new bytes.Matcher(buf1), new bytes.Matcher(buf2));
};
...
Type.prototype.binaryDecode = function (buf, resolver, noCheck) {
  var decoder = new BinaryDecoder(buf);
  var val = decoder.decode(this, resolver, noCheck);
  if (val === undefined) {
    throw new Error('truncated buffer');
  }
  if (!noCheck && decoder.index < buf.length) {
...
Type.prototype.binaryEncode = function (val) {
  WRITER.index = 0;
  this._write(WRITER, val);
  var buf = WRITER.buffer;
  var idx = WRITER.index;
  if (idx > buf.length) {
    // TODO: Resize.
  }
  var encoded = new Buffer(idx);
  buf.copy(encoded, 0, 0, idx);
  return buf;
};

Type.prototype.jsonDecode = function (obj, opts) {
  // TODO
  opts = {
    coerce: !!opts.tryCoerceBuffers | 0, // Coerce JSON to Buffer.
    qualifyNames: !!opts.tryQualifyNames,
    wrap: !!opts.tryWrapUnions | 0 // Wrap first match into union.
  };
  return this._copy(val, opts);
};

Type.prototype.jsonEncode = function (val) {
  return val; // TODO
};
...
NullType.prototype._write = function (writer, val) {
  if (val !== null) {
...
BooleanType.prototype._read = function (reader) {
  return reader.readBoolean();
};

BooleanType.prototype._skip = function (reader) { reader.skipBoolean(); };

BooleanType.prototype._write = function (writer, val) {
...
BooleanType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchBoolean(matcher2);
};
...
IntType.prototype._read = function (reader) { return reader.readInt(); };

IntType.prototype._skip = function (reader) { reader.skipInt(); };

IntType.prototype._write = function (writer, val) {
  if (val !== (val | 0)) {
    throwInvalidError(val, this);
  }
  writer.writeInt(val);
...
IntType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchInt(matcher2);
};
 ...
 * and throws rather than potentially silently change the data. See `forMethods` or
...
LongType.prototype._read = function (reader) {
...
LongType.prototype._skip = function (reader) { reader.skipLong(); };

LongType.prototype._write = function (writer, val) {
...
LongType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchLong(matcher2);
};
...
LongType.forMethods = function (methods, opts) {
  methods = methods || {}; // Will give a more helpful error message.
  // We map some of the methods to a different name to be able to intercept
  // their input and output (otherwise we wouldn't be able to perform any
  // unpacking logic, and the type wouldn't work when nested).
  var mapping = {
    binaryDecode: '_fromBuffer',
    binaryEncode: '_toBuffer',
    jsonDecode: '_fromJSON',
    jsonEncode: '_toJSON',
    isValid: '_isValid',
    compare: 'compare'
  };
  var type = new AbstractLongType(opts && opts.noUnpack);
...
FloatType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchFloat(matcher2);
};
...
DoubleType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchDouble(matcher2);
};
...
StringType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchString(matcher2);
};
...
BytesType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchBytes(matcher2);
};
...
function UnionType(schema, opts) {
  ...
  this.types = schema.map(function (obj) {
    return Type.forSchema(obj, opts);
  });
...
UnionType.prototype._match = function (matcher1, matcher2) {
  var n1 = matcher1.readLong();
  var n2 = matcher2.readLong();
  if (n1 === n2) {
    return this.types[n1]._match(matcher1, matcher2);
  } else {
    return n1 < n2 ? -1 : 1;
  }
};
...
EnumType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchLong(matcher2);
};
...
FixedType.prototype._match = function (matcher1, matcher2) {
  return matcher1.matchFixed(matcher2, this.size);
};
...
ArrayType.prototype._match = function (matcher1, matcher2) {
  var n1 = matcher1.readLong();
  var n2 = matcher2.readLong();
  var f;
  while (n1 && n2) {
    f = this.itemsType._match(matcher1, matcher2);
    if (f) {
      return f;
    }
    if (!--n1) {
      n1 = readArraySize(matcher1);
    }
    if (!--n2) {
      n2 = readArraySize(matcher2);
    }
  }
...
function RecordType(schema, opts) {
  ...
  this._fieldsByName = {};
  this.fields = schema.fields.map(function (f) {
...
RecordType.prototype._match = function (matcher1, matcher2) {
  ...
  for (i = 0, l = fields.length; i < l; i++) {
    ...
    type = field.type;
    if (order) {
      order *= type._match(matcher1, matcher2);
      if (order) {
        return order;
      }
    } else {
...
LogicalType.prototype._equals = utils.abstractFunction;
 ...
 * `Number.MAX_SAFE_INTEGER`). See `LongType.forMethods` method above. Note
 * that we can't use a logical type because we need a "lower-level" hook here:
 * passing through through the standard long would cause a loss of precision.
 */
...
AbstractLongType.prototype._read = function (reader) {
  var buf, pos;
  if (this._noUnpack) {
    pos = reader.pos;
    reader.skipLong();
    buf = reader.buf.slice(pos, reader.pos);
  } else {
    buf = reader.unpackLongBytes(reader);
  }
  if (reader.isValid()) {
...
AbstractLongType.prototype._write = function (writer, val) {
  if (!this._isValid(val)) {
    throwInvalidError(val, this);
  }
  var buf = this._toBuffer(val);
  if (this._noUnpack) {
    writer.writeFixed(buf);
  } else {
    writer.packLongBytes(buf);
  }
};
...
BinaryDecoder.prototype.decode = function (type, resolver, noAdvance) {
  var reader = this._reader;
  var val;
  if (resolver) {
    val = resolver._read(reader, noAdvance);
  } else {
    val = type._read(reader);
  }
...
function BinaryEncoder(buf, idx) {
  this._writer = new Writer(buf, idx);
  this.index = idx;
}

BinaryEncoder.prototype.encode = function (val, type) {
  var writer = this._writer;
  type._write(writer);
  var newIndex = writer.index;
  var remaining = writer.buffer.length - newIndex;
  if (remaining >= 0) {
    // Write was valid.
    this.index = newIndex;
  }
