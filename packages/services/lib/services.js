...
Adapter.prototype._decodeRequest = function (buf) {
  var hdrObj = MAP_BYTES_TYPE.decode(buf, 0);
  var nameObj = STRING_TYPE.decode(buf, hdrObj.offset);
  var name = nameObj.value;
  var msg, reqObj, req;
  if (name) {
    msg = this._serverSvc.message(name);
    reqObj = this._readers[name + '?'].decode(buf, nameObj.offset);
    req = reqObj.value;
  } else {
    msg = PING_MESSAGE;
  }
  if ((reqObj ? reqObj : nameObj).offset < 0) {
    throw new Error(f('truncated %s request', name || 'ping$'));
  }
  return new WrappedRequest(msg, hdrObj.value, req);
};

Adapter.prototype._decodeResponse = function (buf, wres, msg) {
  var hdrObj = MAP_BYTES_TYPE.decode(buf, 0);
  if (hdrObj.value) {
    utils.copyOwnProperties(hdrObj.value, wres.headers, true);
  }
  var isErrorObj = BOOLEAN_TYPE.decode(buf, hdrObj.offset);
  var isError = isErrorObj.value;
  var name = msg.name;
  if (name) {
    ...
    var obj;
    if (isError) {
      obj = reader.decode(buf, isErrorObj.offset);
      wres.error = obj.value;
    } else {
      obj = reader.decode(buf, isErrorObj.offset);
      wres.response = obj.value;
    }
    if (obj.offset < 0) {
...
function readHead(type, buf) {
  var obj = type.decode(buf, 0);
  if (obj.offset < 0) {
    throw new Error(f('truncated %s', type));
  }
  return {head: obj.value, tail: buf.slice(obj.offset)};
}
