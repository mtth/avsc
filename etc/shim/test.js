...
/**
 * Tests lifted from the previous implementation.
 *
 * This file is almost exactly identical to the previous one, except for 5
 * assertions (see lines below with "Changed" comment).
 *
 */
...
exports.test = {
  ...
  'null': function(test) {
    ...
    test.throws(function () { Validator.validate('null', undefined); }); // Changed (Incorrect.)
  ...
  'long': function(test) {
    ...
    test.ok(Validator.validate('long', Math.pow(2, 48) - 1)); // Changed (Dangerous to allow rounded values.)
  ...
  'record-with-string': function(test) {
    ...
    test.ok(Validator.validate(schema, {stringField: 'a', unexpectedField: 'a'})); // Changed. (Too expensive and inconvenient for clients, this would limit object reuse.)
    test.done();
  },
  'record-with-string-and-number': function(test) {
    ...
    test.ok(Validator.validate(schema, {stringField: 'a', intField: 1})); // Changed. (Idem.)
    test.done();
  },
  ...
  'protocol': function(test) {
    ...
    test.throws(function() { ProtocolValidator.validate(protocol, 'RecordA', {}); }); // Changed. (Unqualified name lookup leads to confusing behavior.)
    test.ok(ProtocolValidator.validate(protocol, 'x.y.z.RecordA', {}));
    test.throws(function() { ProtocolValidator.validate(protocol, 'RecordB', {}); }); // Changed. (Unqualified name lookup leads to confusing behavior.)
